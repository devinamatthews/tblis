#ifndef _TENSOR_UTIL_STACK_ALLOCATOR_HPP_
#define _TENSOR_UTIL_STACK_ALLOCATOR_HPP_
...
class _stack
{
    public:
        ...
        _stack(_stack&&) = default;

        _stack& operator=(const _stack&) = delete;

        _stack& operator=(_stack&&) = default;
        ...
        template <typename T>
        T* allocate(size_t size, size_t align = alignof(T))
        {
        ...
        template <typename T>
        bool deallocate(T* t_ptr, size_t size)
        {
            size_t size_bytes = size*sizeof(T);
            char *ptr = (char*)t_ptr;
    ...
    protected:
        ...
        {
            return pos + (align-1) - (pos-1)%align;
        }
...
class stack_allocator
{
    public:
        ...
        : _stack_size(stack_size), _expandable(expandable), _align(alignment), _stack(0)
        {
        ...
        T* allocate(size_t size)
        {
            ...
            T* ptr = _stacks->at(_stack).allocate<T>(size, _align);
            ...
            if (!ptr)
            {
                ...
                ptr = _stacks->at(_stack).allocate<T>(size, _align);
                if (!ptr)
    ...
    protected:
        ...
        bool _expandable;
        size_t _align;
        std::shared_ptr<std::vector<_stack>> _stacks;
        int _stack;
};
