...
AS_VAR_PUSHDEF([ac_Type], [ax_cv_decl_${typename}_signed])
AC_CACHE_CHECK([whether $1 is signed], ac_Type,
[AC_COMPILE_IFELSE([AC_LANG_PROGRAM([AC_INCLUDES_DEFAULT([$4])],
[int foo @<:@ 1 - 2 * !((($1) -1) < 0) @:>@;])],
                   [AS_VAR_SET(ac_Type, yes)],
                   [AS_VAR_SET(ac_Type, no)])])
AS_IF([test AS_VAR_GET(ac_Type) = yes], [$2], [$3])
AS_VAR_POPDEF([ac_Type])
])dnl
