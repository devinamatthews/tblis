...
template <typename T>
static void tblis_axpyv_ref(bool conj_A, dim_t n,
...
template <typename T>
static void tblis_axpyv_int(T alpha, const Matrix<T>& A, Matrix<T>& B)
{
    if (A.length() > 1)
    {
        tblis_axpyv_ref(A.is_conjugated(), A.length(), alpha, A.data(), A.row_stride(),
                                                              B.data(), B.row_stride());
    }
    else
    {
        tblis_axpyv_ref(A.is_conjugated(), A.width(), alpha, A.data(), A.col_stride(),
                                                             B.data(), B.col_stride());
    }
}
...
void tblis_axpyv(T alpha, const Matrix<T>& A, Matrix<T>& B)
{
    ...
    tblis_axpyv_int(alpha, A, B);
}

#define INSTANTIATION(T,MT,NT,KT,MR,NR,KR) \
template void tblis_axpyv(bool conj_A, dim_t n, T alpha, const T* A, inc_t inc_A, T* B, inc_t inc_B); \
template void tblis_axpyv(T alpha, const Matrix<T>& A, Matrix<T>& B);
