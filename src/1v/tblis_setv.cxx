...
template <typename T>
void tblis_setv_ref(thread_communicator& comm, len_type n, T alpha, T* A, stride_type inc_A)
{
    if (n == 0) return;
...
template <typename T>
void tblis_setv(len_type n, T alpha, T* A, stride_type inc_A)
{
    if (alpha == T(0))
    {
