...
#define bli_xpbys_mxn(ch, m, n, a_, rs_a, cs_a, beta_, b_, rs_b, cs_b)       \
    ...
    {                                                                        \
        ...
        {                                                                    \
            for (dim_t i = 0; i < m; i++)                                    \
                for (dim_t j = 0; j < n; j++)                                \
                    b[i * (rs_b) + j * (cs_b)] = beta                        \
                                               * b[i * (rs_b) + j * (cs_b)]  \
                                               + a[i * (rs_a) + j * (cs_a)]; \
        }                                                                    \
    } while (0)

#define bli_sxpbys_mxn(m, n, a_, rs_a, cs_a, beta_, b_, rs_b, cs_b) \
    bli_xpbys_mxn(s, m, n, a_, rs_a, cs_a, beta_, b_, rs_b, cs_b)
#define bli_dxpbys_mxn(m, n, a_, rs_a, cs_a, beta_, b_, rs_b, cs_b) \
    bli_xpbys_mxn(d, m, n, a_, rs_a, cs_a, beta_, b_, rs_b, cs_b)
#define bli_cxpbys_mxn(m, n, a_, rs_a, cs_a, beta_, b_, rs_b, cs_b) \
    bli_xpbys_mxn(c, m, n, a_, rs_a, cs_a, beta_, b_, rs_b, cs_b)
#define bli_zxpbys_mxn(m, n, a_, rs_a, cs_a, beta_, b_, rs_b, cs_b) \
    bli_xpbys_mxn(z, m, n, a_, rs_a, cs_a, beta_, b_, rs_b, cs_b)
...
#define GEMM_UKR_SETUP_CT_PRE(ch, mr, nr, row_major, alignment) \
    ...
    PASTEMAC(ch, ctype)                                         \
    _ct[BLIS_STACK_BUF_MAX_SIZE / sizeof(PASTEMAC(ch, ctype))]  \
...
#define GEMM_UKR_SETUP_CT(ch, mr, nr, row_major)                            \
    ...
    const bool _use_ct =                                                    \
        (row_major ? cs_c != 1 : rs_c != 1) || m != mr || n != nr;          \
...
#define GEMM_UKR_SETUP_CT_AMBI(ch, mr, nr, row_major)                     \
    ...
    GEMM_UKR_SETUP_CT_PRE(ch, mr, nr, row_major, 1);                      \
    const bool _use_ct = (cs_c != 1 && rs_c != 1) || m != mr || n != nr;  \
    GEMM_UKR_SETUP_CT_POST(ch);
...
#define GEMM_UKR_SETUP_CT_ALIGNED(ch, mr, nr, row_major, alignment)       \
    ...
    const bool _use_ct =                                                  \
        (row_major ? cs_c != 1 : rs_c != 1)                               \
        || m                                                              \
        != mr                                                             \
        || n                                                              \
        != nr                                                             \
        || ((uintptr_t)_c % alignment)                                    \
        || (((row_major ? _rs_c : _cs_c) * sizeof(PASTEMAC(ch, ctype)))   \
            % alignment);                                                 \
    GEMM_UKR_SETUP_CT_POST(ch);
...
#define GEMM_UKR_FLUSH_CT(ch)                                            \
                                                                         \
    /* If we actually used the temporary microtile, accumulate it to the \
       output microtile. */                                                           \
    if (_use_ct)                                                         \
    {                                                                    \
        PASTEMAC(ch, xpbys_mxn)                                          \
        (m, n, _ct, _rs_ct, _cs_ct, _beta, _c, _rs_c, _cs_c);            \
    }

#define EXTERN_BLIS_GEMM_UKR(name)        \
    extern "C" void name(dim_t k,         \
                         void* alpha,     \
                         void* a,         \
                         void* b,         \
                         void* beta,      \
...
using blis_gemm_ukr_t = void (*)(dim_t k,
                                 void* alpha,
                                 void* a,
                                 void* b,
                                 void* beta,
...
void blis_wrap_gemm_ukr(stride_type m,
                        stride_type n,
                        stride_type k,
                        const void* p_a,
                        const void* p_b,
                        ...
                        auxinfo_t* aux)
{
    ...
    else
    {
        ...
        Config::template update_nn_ukr<T>::value(m,
                                                 n,
                                                 p_ab,
                                                 nullptr,
                                                 0,
