...
template <typename T> struct is_row<marray_view<T, 1>> : std::true_type
{
};
...
struct is_row_of<
    T,
    U,
    std::enable_if_t<is_row<T>::value
...
: std::integral_constant<bool,
                         is_row_of<T, U>::value || is_container_of<T, U>::value>
{
};

template <typename T, typename U, typename V = void>
using enable_if_1d_container_of_t =
    std::enable_if_t<is_1d_container_of<T, U>::value, V>;
...
/**
 * Adaptor class which can capture a container, initializer_list, @ref row, or
 * @ref row_view with elements convertible to a specified type.
 *
 ...
 *
 * // An initializer_list<int> (sort of...actually narrowing conversions are
 * allowed): foo({1, 4, 4l, 3u});
 *
 * // A linear container:
...
template <typename T> class array_1d
{
  protected:
    ...
    template <typename U> struct adaptor : adaptor_base
    {
        ...
        adaptor(U data)
        : adaptor_base(detail::length(data)),
          data(std::move(data))
        {
        }
        ...
        {
            return *(new (static_cast<adaptor*>(&other))
                         adaptor(std::move(*this)));
        }
    };

    static constexpr size_t _s1 = sizeof(adaptor<T*&>);
    static constexpr size_t _s2 =
        sizeof(adaptor<short_vector<T, MARRAY_OPT_NDIM>>);
  ...
  public:
    ...
    : adaptor_(
          other.adaptor_.copy(reinterpret_cast<adaptor_base&>(raw_adaptor_)))
    {
    }

    array_1d(array_1d&& other)
    : adaptor_(
          other.adaptor_.move(reinterpret_cast<adaptor_base&>(raw_adaptor_)))
    {
    }
    ...
    array_1d(const U& data) : adaptor_(adapt(data))
    {
    }

    ~array_1d() { adaptor_.~adaptor_base(); }

    template <size_t N> void slurp(std::array<T, N>& x) const
    {
    ...
    template <size_t N> void slurp(short_vector<T, N>& x) const
    {
        x.resize(size());
        adaptor_.slurp(x.data());
    }
