...
template <typename T> struct is_matrix<marray_view<T, 2>> : std::true_type
{
};
...
struct is_matrix_of<
    T,
    U,
    std::enable_if_t<is_matrix<T>::value
                     && std::is_assignable<U&, typename T::value_type>::value>>
: std::true_type
{
};
...
using enable_if_2d_container_of_t =
    std::enable_if_t<is_2d_container_of<T, U>::value, V>;
...
/**
 * Adaptor class which can capture a 2-d data structure: a container of
 * containers, @ref matrix or @ref matrix_view, nested initializer list, etc.
 * which have elements convertible to a specified type.
 *
 * This class is used to capture a variety of acceptable argument types without
 * relying on template parameters. For example:
 *
 * @code{.cxx}
 * void foo(const array_2d<int>&);
 *
 * // An initializer_list<initializer_list<int>> (sort of...actually narrowing
 * conversions are allowed): foo({{1, 4}, {4l, 3u}});
 *
 ...
 * matrix_view<int> mv({2, 2}, v.data(), ROW_MAJOR); // view v as a 2-d
 * structure: {{6, 3}, {0, 1}} foo(mv);
 * @endcode
 *
 * @tparam T  Elements of the supplied container, initializer list, etc. must be
...
template <typename T> class array_2d
{
  protected:
    ...
    template <typename U> struct adaptor : adaptor_base
    {
        static constexpr bool IsMatrix =
            detail::is_matrix<typename std::decay<U>::type>::value;
        ...
        do_slurp(T* x, len_type rs, len_type cs) const
        {
            ...
            {
                int j = 0;
                for (auto it2 = it->begin(), end2 = it->end(); it2 != end2;
                     ++it2)
                {
                    x[i * rs + j * cs] = *it2;
                    j++;
        ...
        do_slurp(std::vector<std::vector<T>>& x) const
        {
            ...
            {
                x[i].resize(len[1]);
                for (len_type j = 0; j < len[1]; j++) { x[i][j] = data[i][j]; }
            }
        }
    ...
    static constexpr size_t _s1 =
        sizeof(adaptor<std::initializer_list<std::initializer_list<int>>>);
  ...
  public:
    array_2d(const array_2d& other) : adaptor_(adapt(other)) {}
    ...
    template <
        typename U,
        typename = std::enable_if_t<detail::is_1d_container_of<U, T>::value>>
    ...
    template <
        typename U,
        typename = std::enable_if_t<detail::is_2d_container_of<U, T>::value>>
    array_2d(const U& data) : adaptor_(adapt<const U&>(data))
    {
    }
