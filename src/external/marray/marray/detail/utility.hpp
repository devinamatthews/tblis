...
template <typename T, typename U> T ipow(T a, U b)
{
    T ab = 1;
    while (b-- > 0) ab *= a;
    return ab;
 ...
 * @ingroup util
 */
template <typename T> void divide(T x, T y, T& q, T& r)
{
...
template <typename T> T inverse_permutation(const T& p)
{
    T ip(p.size());
...
template <typename T, typename... Args> struct are_convertible;

template <typename T> struct are_convertible<T> : std::true_type
{
};
...
template <typename T, typename... Args> struct are_assignable;

template <typename T> struct are_assignable<T> : std::true_type
{
};
...
template <typename T>
struct is_index_or_slice_helper<
    T,
    std::enable_if_t<std::is_convertible_v<T, len_type>>> : std::true_type
{
};
...
template <> struct is_index_or_slice_helper<all_t> : std::true_type
{
};

template <> struct is_index_or_slice_helper<bcast_t> : std::true_type
{
};
...
template <typename... Args> struct are_indices_or_slices;

template <> struct are_indices_or_slices<> : std::true_type
{
};
...
template <typename... Args> struct count_dimensions;

template <> struct count_dimensions<> : std::integral_constant<int, 0>
{
};
...
using enable_if_container_of_t =
    typename std::enable_if<is_container_of<C, T>::value, U>::type;
...
template <typename C, typename T>
struct is_container_of_containers_of<C,
                                     T,
                                     std::enable_if_t<is_container<C>::value>>
...
using enable_if_container_of_containers_of_t =
    typename std::enable_if<is_container_of_containers_of<C, T>::value,
                            U>::type;

template <typename T, typename... Ts> struct are_containers_helper;

template <typename T> struct are_containers_helper<T> : is_container<T>
{
};

template <typename T, typename... Ts>
struct are_containers_helper : std::conditional<is_container<T>::value,
                                                are_containers_helper<Ts...>,
                                                std::false_type>::type
{
};

template <typename... Ts> struct are_containers;

template <> struct are_containers<> : std::true_type
{
};

template <typename... Ts> struct are_containers : are_containers_helper<Ts...>
{
};
...
template <typename T, typename... Cs> struct are_containers_of;

template <typename T> struct are_containers_of<T> : std::true_type
{
};
...
template <typename Iterator> void inc_offsets_helper(int, Iterator) {}

template <typename Iterator, typename Offset, typename... Offsets>
void inc_offsets_helper(int i, Iterator it, Offset& off0, Offsets&... off)
{
    off0 += (*it)[i];
...
template <typename Pos, typename Iterator, typename Offset, typename... Offsets>
void
move_offsets_helper(const Pos& pos, Iterator it, Offset& off0, Offsets&... off)
{
...
void
set_strides_helper(Iterator it, const Stride& stride, const Strides&... strides)
{
...
template <typename T, typename = void> struct wrap_if_integral_helper
{
    using type = T;
};
...
template <typename Type, int NDim> struct initializer_type;

template <typename Type> struct initializer_type<Type, 0>
{
    typedef Type type;
};

template <typename Type> struct initializer_type<Type, DYNAMIC>
{
    struct private_type
    {
    };
...
template <typename Type, int NDim> struct initializer_type
{
    typedef std::initializer_list<
        typename initializer_type<Type, NDim - 1>::type>
        type;
};
...
template <typename T, size_t N> struct array : std::array<T, N>
{
    ...
    void clear() { static_cast<std::array<T, N>&>(*this) = {}; }
};

template <typename T, int NDim> struct array_type
{
    typedef array<T, NDim> type;
};

template <typename T> struct array_type<T, DYNAMIC>
{
