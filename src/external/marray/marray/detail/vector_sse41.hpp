...
template <> struct vector_traits<float>
{
    ...
    template <typename T>
    static std::enable_if_t<std::is_same<T, uint8_t>::value, __m128i>
    convert(__m128 v)
    {
        __m128i i32 = _mm_cvtps_epi32(v);
    ...
    template <typename T>
    static std::enable_if_t<std::is_same<T, int16_t>::value, __m128i>
    convert(__m128 v)
    {
        __m128i i32 = _mm_cvtps_epi32(v);
    ...
    template <typename T>
    static std::enable_if_t<std::is_same<T, uint16_t>::value, __m128i>
    convert(__m128 v)
    {
        __m128i i32 = _mm_cvtps_epi32(v);
    ...
    convert(__m128 v)
    {
        return _mm_setr_epi32((uint32_t)v[0],
                              (uint32_t)v[1],
                              (uint32_t)v[2],
                              (uint32_t)v[3]);
    }
    ...
    static __m128 set1(float val) { return _mm_set1_ps(val); }
    ...
    static __m128 negate(__m128 a) { return _mm_xor_ps(a, _mm_set1_ps(-0.0f)); }
...
template <> struct vector_traits<double>
{
    ...
    static __m128d set1(double val) { return _mm_set1_pd(val); }
    ...
    static __m128d sqrt(__m128d a) { return _mm_sqrt_pd(a); }
};
...
template <> struct vector_traits<std::complex<float>>
{
    ...
    static __m128 mul(__m128 a, __m128 b)
    {
        ...
        __m128 tmp1 =
            _mm_mul_ps(a, breal); // tmp1 = (ar0*br0, ai0*br0, ar1*br1, ai1*br1)
        __m128 tmp2 =
            _mm_mul_ps(ashuf,
    ...
    static __m128 div(__m128 a, __m128 b)
    {
        ...
        return _mm_setr_ps(c0.real(), c0.imag(), c1.real(), c1.imag());
    }
    ...
    static __m128 exp(__m128 a)
    {
        ...
        return _mm_setr_ps(b0.real(), b0.imag(), b1.real(), b1.imag());
    }
                                      ...
                                      || std::is_same<U, uint8_t>::value>>
{
    ...
    static __m128i set1(U val) { return _mm_set1_epi8(val); }
    ...
    static __m128i sub(__m128i a, __m128i b) { return _mm_sub_epi8(a, b); }
    ...
    static __m128i div(__m128i a, __m128i b)
    {
        return _mm_setr_epi8(
            (U)_mm_extract_epi8(a, 0) / (U)_mm_extract_epi8(b, 0),
            (U)_mm_extract_epi8(a, 1) / (U)_mm_extract_epi8(b, 1),
            (U)_mm_extract_epi8(a, 2) / (U)_mm_extract_epi8(b, 2),
            (U)_mm_extract_epi8(a, 3) / (U)_mm_extract_epi8(b, 3),
            (U)_mm_extract_epi8(a, 4) / (U)_mm_extract_epi8(b, 4),
            (U)_mm_extract_epi8(a, 5) / (U)_mm_extract_epi8(b, 5),
            (U)_mm_extract_epi8(a, 6) / (U)_mm_extract_epi8(b, 6),
            (U)_mm_extract_epi8(a, 7) / (U)_mm_extract_epi8(b, 7),
            (U)_mm_extract_epi8(a, 8) / (U)_mm_extract_epi8(b, 8),
            (U)_mm_extract_epi8(a, 9) / (U)_mm_extract_epi8(b, 9),
            (U)_mm_extract_epi8(a, 10) / (U)_mm_extract_epi8(b, 10),
            (U)_mm_extract_epi8(a, 11) / (U)_mm_extract_epi8(b, 11),
            (U)_mm_extract_epi8(a, 12) / (U)_mm_extract_epi8(b, 12),
            (U)_mm_extract_epi8(a, 13) / (U)_mm_extract_epi8(b, 13),
            (U)_mm_extract_epi8(a, 14) / (U)_mm_extract_epi8(b, 14),
            (U)_mm_extract_epi8(a, 15) / (U)_mm_extract_epi8(b, 15));
    }

    static __m128i pow(__m128i a, __m128i b)
    {
        return _mm_setr_epi8(
            (U)std::pow((U)_mm_extract_epi8(a, 0), (U)_mm_extract_epi8(b, 0)),
            (U)std::pow((U)_mm_extract_epi8(a, 1), (U)_mm_extract_epi8(b, 1)),
            (U)std::pow((U)_mm_extract_epi8(a, 2), (U)_mm_extract_epi8(b, 2)),
            (U)std::pow((U)_mm_extract_epi8(a, 3), (U)_mm_extract_epi8(b, 3)),
            (U)std::pow((U)_mm_extract_epi8(a, 4), (U)_mm_extract_epi8(b, 4)),
            (U)std::pow((U)_mm_extract_epi8(a, 5), (U)_mm_extract_epi8(b, 5)),
            (U)std::pow((U)_mm_extract_epi8(a, 6), (U)_mm_extract_epi8(b, 6)),
            (U)std::pow((U)_mm_extract_epi8(a, 7), (U)_mm_extract_epi8(b, 7)),
            (U)std::pow((U)_mm_extract_epi8(a, 8), (U)_mm_extract_epi8(b, 8)),
            (U)std::pow((U)_mm_extract_epi8(a, 9), (U)_mm_extract_epi8(b, 9)),
            (U)std::pow((U)_mm_extract_epi8(a, 10), (U)_mm_extract_epi8(b, 10)),
            (U)std::pow((U)_mm_extract_epi8(a, 11), (U)_mm_extract_epi8(b, 11)),
            (U)std::pow((U)_mm_extract_epi8(a, 12), (U)_mm_extract_epi8(b, 12)),
            (U)std::pow((U)_mm_extract_epi8(a, 13), (U)_mm_extract_epi8(b, 13)),
            (U)std::pow((U)_mm_extract_epi8(a, 14), (U)_mm_extract_epi8(b, 14)),
                                      ...
                                      || std::is_same<U, uint16_t>::value>>
{
    ...
    static __m128i set1(U val) { return _mm_set1_epi16(val); }
    ...
    static __m128i sub(__m128i a, __m128i b) { return _mm_sub_epi16(a, b); }

    static __m128i mul(__m128i a, __m128i b) { return _mm_mullo_epi16(a, b); }

    static __m128i div(__m128i a, __m128i b)
    {
        return _mm_setr_epi16(
            (U)_mm_extract_epi16(a, 0) / (U)_mm_extract_epi16(b, 0),
            (U)_mm_extract_epi16(a, 1) / (U)_mm_extract_epi16(b, 1),
            (U)_mm_extract_epi16(a, 2) / (U)_mm_extract_epi16(b, 2),
            (U)_mm_extract_epi16(a, 3) / (U)_mm_extract_epi16(b, 3),
            (U)_mm_extract_epi16(a, 4) / (U)_mm_extract_epi16(b, 4),
            (U)_mm_extract_epi16(a, 5) / (U)_mm_extract_epi16(b, 5),
            (U)_mm_extract_epi16(a, 6) / (U)_mm_extract_epi16(b, 6),
            (U)_mm_extract_epi16(a, 7) / (U)_mm_extract_epi16(b, 7));
    }

    static __m128i pow(__m128i a, __m128i b)
    {
        return _mm_setr_epi16(
            (U)std::pow((U)_mm_extract_epi16(a, 0), (U)_mm_extract_epi16(b, 0)),
            (U)std::pow((U)_mm_extract_epi16(a, 1), (U)_mm_extract_epi16(b, 1)),
            (U)std::pow((U)_mm_extract_epi16(a, 2), (U)_mm_extract_epi16(b, 2)),
            (U)std::pow((U)_mm_extract_epi16(a, 3), (U)_mm_extract_epi16(b, 3)),
            (U)std::pow((U)_mm_extract_epi16(a, 4), (U)_mm_extract_epi16(b, 4)),
            (U)std::pow((U)_mm_extract_epi16(a, 5), (U)_mm_extract_epi16(b, 5)),
            (U)std::pow((U)_mm_extract_epi16(a, 6), (U)_mm_extract_epi16(b, 6)),
                                      ...
                                      || std::is_same<U, uint32_t>::value>>
{
    ...
    static __m128i set1(U val) { return _mm_set1_epi32(val); }
    ...
    static __m128i sub(__m128i a, __m128i b) { return _mm_sub_epi32(a, b); }

    static __m128i mul(__m128i a, __m128i b) { return _mm_mullo_epi32(a, b); }

    static __m128i div(__m128i a, __m128i b)
    {
        return _mm_setr_epi32(
            (U)_mm_extract_epi32(a, 0) / (U)_mm_extract_epi32(b, 0),
            (U)_mm_extract_epi32(a, 1) / (U)_mm_extract_epi32(b, 1),
            (U)_mm_extract_epi32(a, 2) / (U)_mm_extract_epi32(b, 2),
            (U)_mm_extract_epi32(a, 3) / (U)_mm_extract_epi32(b, 3));
    }

    static __m128i pow(__m128i a, __m128i b)
    {
        return _mm_setr_epi32(
            (U)std::pow((U)_mm_extract_epi32(a, 0), (U)_mm_extract_epi32(b, 0)),
            (U)std::pow((U)_mm_extract_epi32(a, 1), (U)_mm_extract_epi32(b, 1)),
            (U)std::pow((U)_mm_extract_epi32(a, 2), (U)_mm_extract_epi32(b, 2)),
                                      ...
                                      || std::is_same<U, uint64_t>::value>>
{
    ...
    static __m128i set1(U val) { return _mm_set1_epi64x(val); }
    ...
    static __m128i sub(__m128i a, __m128i b) { return _mm_sub_epi64(a, b); }

    static __m128i mul(__m128i a, __m128i b)
    {
        return _mm_set_epi64x(
            (U)_mm_extract_epi64(a, 1) * (U)_mm_extract_epi64(b, 1),
            (U)_mm_extract_epi64(a, 0) * (U)_mm_extract_epi64(b, 0));
    }
    ...
    {
        return _mm_set_epi64x(
            (U)std::pow((U)_mm_extract_epi64(a, 1), (U)_mm_extract_epi64(b, 1)),
