...
class dpd_marray
: public dpd_marray_base<Type, dpd_marray<Type, Allocator>, true>
{
  protected:
  ...
  public:
    ...
    dpd_marray(const dpd_marray& other) { reset(other); }
    ...
    void reset(dpd_marray&& other) { swap(other); }

    template <typename U, typename A> void reset(const dpd_marray<U, A>& other)
    {
        reset();
    ...
    friend void swap(dpd_marray& a, dpd_marray& b) { a.swap(b); }
};
