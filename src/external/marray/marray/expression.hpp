...
using expression_type_t = typename expression_type<Expr>::type;
...
template <typename T, typename... Dims> struct array_expr
{
    ...
    array_expr(T* data, const Dims&... dims) : data(data), dims{dims...} {}

    result_type eval() const { return *data; }
    ...
    result_type eval_at(len_type i, const slice_dim&) const { return data[i]; }

    result_type eval_at(len_type, const bcast_dim&) const { return *data; }
    ...
    {
        return eval_vec<Width, Aligned>(
            i,
            std::get<Dim - (NDim - sizeof...(Dims))>(dims));
    }
...
template <typename Expr>
std::enable_if_t<is_scalar<std::decay_t<Expr>>::value, Expr> eval(Expr&& expr)
{
    return expr;
}
...
template <typename T, typename U, typename = void> struct binary_result_type;

template <typename T, typename U>
struct binary_result_type<T,
                          U,
...
template <typename T, typename U>
struct binary_result_type<std::complex<T>,
                          U,
                          std::enable_if_t<std::is_floating_point<T>::value
                                           && std::is_floating_point<U>::value>>
{
...
struct negate
{
    template <typename T> auto operator()(const T& a) const -> decltype(-a)
    {
        return -a;
    }

    template <typename T> vector_type<T> vec(vector_type<T> a) const
    {
...
template <typename LHS, typename RHS, typename Op> struct binary_expr
{
    ...
    template <int NDim, int Dim> result_type eval_at(len_type i) const
    {
...
template <typename Expr, typename Op> struct unary_expr
{
    ...
    unary_expr(const Expr& expr, const Op& op = Op()) : expr(expr), op(op) {}

    result_type eval() const { return op(MArray::eval(expr)); }
...
template <typename Expr> using exp_expr = unary_expr<Expr, operators::exp>;

template <typename Expr> using log_expr = unary_expr<Expr, operators::log>;

template <typename Expr> using abs_expr = unary_expr<Expr, operators::abs>;

template <typename Expr> using sqrt_expr = unary_expr<Expr, operators::sqrt>;

template <typename T, typename> struct is_array_expression : std::false_type
{
};
...
template <typename T, typename> struct is_expression : std::false_type
{
};
...
template <typename Expr, typename> struct is_unary_expression : std::false_type
{
};

template <typename Expr>
struct is_unary_expression<
    Expr,
...
template <typename Expr, typename> struct is_binary_expression : std::false_type
{
};

template <typename Expr>
struct is_binary_expression<
    Expr,
...
template <typename Array, typename Dim> struct array_expr_type_helper2;
...
template <typename T, int NDim> struct array_expr_helper;

template <typename T> struct array_expr_helper<T, 0>
{
...
template <typename T, int NDim> struct array_expr_helper
{
    typedef typename array_expr_type_helper2<
        typename array_expr_helper<T, NDim - 1>::type,
        slice_dim>::type type;
};

template <typename Array, typename... Dims1> struct slice_array_expr_helper;
...
template <typename Array, typename> struct is_marray
{
    template <typename T, int NDim, typename Derived, bool Owner>
    static std::bool_constant<NDim != DYNAMIC>
    check(const marray_base<T, NDim, Derived, Owner>*);

    template <typename T, int NDim, int NIndexed, typename... Dims>
    static std::true_type
    check(const marray_slice<T, NDim, NIndexed, Dims...>*);
...
{
    template <typename T, int NDim, int NIndexed, typename... Dims>
    static typename slice_array_expr_helper<
        typename array_expr_helper<T, NDim - NIndexed>::type,
        Dims...>::type
    check(const marray_slice<T, NDim, NIndexed, Dims...>*);

    template <typename T, int NDim, typename Derived>
    static typename array_expr_helper<T, NDim>::type
    check(const marray_base<T, NDim, Derived, false>*);

    template <typename T, int NDim, typename Derived>
    static typename array_expr_helper<const T, NDim>::type
    check(const marray_base<T, NDim, Derived, true>*);

    template <typename T, int NDim, typename Derived>
    static typename array_expr_helper<T, NDim>::type
    check(marray_base<T, NDim, Derived, true>*);
...
: std::integral_constant<bool,
                         is_expression<std::decay_t<Expr>>::value
...
: std::integral_constant<bool,
                         is_expression_arg<std::decay_t<Expr>>::value
                             || is_scalar<std::decay_t<Expr>>::value>
{
};
...
struct converted_scalar_type<
    T,
    Expr,
    std::enable_if_t<is_scalar<T>::value && is_expression_arg<Expr>::value>>
...
template <typename LHS, typename RHS>
std::enable_if_t<
    is_expression_arg<LHS>::value && is_expression_arg<RHS>::value,
    add_expr<expression_type_t<const LHS>, expression_type_t<const RHS>>>
...
template <typename LHS, typename RHS>
std::enable_if_t<
    is_expression_arg<LHS>::value && is_scalar<RHS>::value,
    add_expr<expression_type_t<const LHS>, converted_scalar_type_t<RHS, LHS>>>
operator+(const LHS& lhs, const RHS& rhs)
{
...
template <typename LHS, typename RHS>
std::enable_if_t<
    is_scalar<LHS>::value && is_expression_arg<RHS>::value,
    add_expr<converted_scalar_type_t<LHS, RHS>, expression_type_t<const RHS>>>
operator+(const LHS& lhs, const RHS& rhs)
{
...
std::enable_if_t<
    is_expression_arg<LHS>::value && is_expression_arg<RHS>::value,
    sub_expr<expression_type_t<const LHS>, expression_type_t<const RHS>>>
operator-(const LHS& lhs, const RHS& rhs)
{
    return {make_expression(lhs), make_expression(rhs)};
}
...
std::enable_if_t<
    is_expression_arg<LHS>::value && is_scalar<RHS>::value,
    sub_expr<expression_type_t<const LHS>, converted_scalar_type_t<RHS, LHS>>>
operator-(const LHS& lhs, const RHS& rhs)
{
    typedef converted_scalar_type_t<RHS, LHS> T;
    return {make_expression(lhs), (T)rhs};
...
std::enable_if_t<
    is_scalar<LHS>::value && is_expression_arg<RHS>::value,
    sub_expr<converted_scalar_type_t<LHS, RHS>, expression_type_t<const RHS>>>
operator-(const LHS& lhs, const RHS& rhs)
{
    typedef converted_scalar_type_t<LHS, RHS> T;
    return {(T)lhs, make_expression(rhs)};
...
std::enable_if_t<
    is_expression_arg<LHS>::value && is_expression_arg<RHS>::value,
    mul_expr<expression_type_t<const LHS>, expression_type_t<const RHS>>>
operator*(const LHS& lhs, const RHS& rhs)
{
    return {make_expression(lhs), make_expression(rhs)};
}
...
std::enable_if_t<
    is_expression_arg<LHS>::value && is_scalar<RHS>::value,
    mul_expr<expression_type_t<const LHS>, converted_scalar_type_t<RHS, LHS>>>
operator*(const LHS& lhs, const RHS& rhs)
{
    typedef converted_scalar_type_t<RHS, LHS> T;
    return {make_expression(lhs), (T)rhs};
...
std::enable_if_t<
    is_scalar<LHS>::value && is_expression_arg<RHS>::value,
    mul_expr<converted_scalar_type_t<LHS, RHS>, expression_type_t<const RHS>>>
operator*(const LHS& lhs, const RHS& rhs)
{
    typedef converted_scalar_type_t<LHS, RHS> T;
    return {(T)lhs, make_expression(rhs)};
...
std::enable_if_t<
    is_expression_arg<LHS>::value && is_expression_arg<RHS>::value,
    div_expr<expression_type_t<const LHS>, expression_type_t<const RHS>>>
operator/(const LHS& lhs, const RHS& rhs)
{
    return {make_expression(lhs), make_expression(rhs)};
}
...
std::enable_if_t<
    is_expression_arg<LHS>::value && is_scalar<RHS>::value,
    div_expr<expression_type_t<const LHS>, converted_scalar_type_t<RHS, LHS>>>
operator/(const LHS& lhs, const RHS& rhs)
{
    typedef converted_scalar_type_t<RHS, LHS> T;
    return {make_expression(lhs), (T)rhs};
...
std::enable_if_t<
    is_scalar<LHS>::value && is_expression_arg<RHS>::value,
    div_expr<converted_scalar_type_t<LHS, RHS>, expression_type_t<const RHS>>>
operator/(const LHS& lhs, const RHS& rhs)
{
    typedef converted_scalar_type_t<LHS, RHS> T;
    return {(T)lhs, make_expression(rhs)};
...
std::enable_if_t<
    is_expression_arg<LHS>::value && is_expression_arg<RHS>::value,
    pow_expr<expression_type_t<const LHS>, expression_type_t<const RHS>>>
...
std::enable_if_t<
    is_expression_arg<LHS>::value && is_scalar<RHS>::value,
    pow_expr<expression_type_t<const LHS>, converted_scalar_type_t<RHS, LHS>>>
pow(const LHS& lhs, const RHS& rhs)
{
    typedef converted_scalar_type_t<RHS, LHS> T;
    return {make_expression(lhs), (T)rhs};
...
std::enable_if_t<
    is_scalar<LHS>::value && is_expression_arg<RHS>::value,
    pow_expr<converted_scalar_type_t<LHS, RHS>, expression_type_t<const RHS>>>
pow(const LHS& lhs, const RHS& rhs)
{
    typedef converted_scalar_type_t<LHS, RHS> T;
    return {(T)lhs, make_expression(rhs)};
...
{
    return get_array_lengths_helper(
        array,
        std::make_integer_sequence<int, sizeof...(Dims)>());
}
                        ...
                        std::integer_sequence<int, J...>)
{
    return std::array<len_type, NDim>{
        (I - I - 1)...,
        get_expr_length(std::get<J>(array.dims))...};
}

template <int NDim, typename T, typename... Dims>
std::array<len_type, NDim> get_expr_lengths(const array_expr<T, Dims...>& array)
{
    return get_expr_lengths_helper<NDim>(
        array,
...
template <int NDim, int Dim, typename Expr>
std::enable_if_t<is_scalar<Expr>::value, bool> is_contiguous(const Expr&)
{
    return true;
}
...
template <typename Expr, typename = void> struct vector_width;

template <typename T, typename... Dims>
struct vector_width<array_expr<T, Dims...>>
{
    static constexpr int value =
        vector_traits<typename std::remove_cv<T>::type>::vector_width;
};
...
{
    static constexpr int w1_ =
        vector_width<std::decay_t<typename Expr::expr_type>>::value;
    static constexpr int w2_ =
        vector_width<std::decay_t<typename Expr::result_type>>::value;
...
{
    static constexpr int w1_ =
        vector_width<std::decay_t<typename Expr::first_type>>::value;
    static constexpr int w2_ =
        vector_width<std::decay_t<typename Expr::second_type>>::value;
    static constexpr int w3_ =
        vector_width<std::decay_t<typename Expr::result_type>>::value;
    static constexpr int value =
        (w3_ < (w1_ < w2_ ? w1_ : w2_) ? w3_ : (w1_ < w2_ ? w1_ : w2_));
};
 ...
 */
template <int NDim, int Dim, typename Expr>
std::enable_if_t<is_scalar<Expr>::value> increment(const Expr&)
{
}

template <int NDim, int Dim, typename Expr>
std::enable_if_t<is_scalar<Expr>::value> decrement(const Expr&)
{
}
 ...
 */
template <int NDim, int Dim, typename Expr>
std::enable_if_t<is_binary_expression<Expr>::value> increment(Expr& expr)
{
...
template <int NDim, int Dim, typename Expr>
std::enable_if_t<is_binary_expression<Expr>::value> decrement(Expr& expr)
{
...
template <int NDim, int Dim, typename Expr>
std::enable_if_t<is_unary_expression<Expr>::value> increment(Expr& expr)
{
    increment<NDim, Dim>(expr.expr);
}

template <int NDim, int Dim, typename Expr>
std::enable_if_t<is_unary_expression<Expr>::value> decrement(Expr& expr)
{
    decrement<NDim, Dim>(expr.expr);
}

template <typename T, typename U> struct assign_expr_value
{
    void operator()(T& lhs, const U& rhs) const { lhs = rhs; }
};

template <typename T, typename U> struct assign_expr_value<T, std::complex<U>>
{
...
template <int NDim, int Dim = 1> struct assign_expr_loop;

template <int NDim> struct assign_expr_loop<NDim, NDim>
{
    template <typename LHS, typename RHS>
    void
    operator()(LHS& lhs, RHS& rhs, const std::array<len_type, NDim>& len) const
    {
        assign_expr_value<std::decay_t<expr_result_type_t<LHS>>,
                          std::decay_t<expr_result_type_t<RHS>>>
            assign;
...
template <int NDim, int Dim> struct assign_expr_loop
{
    template <typename LHS, typename RHS>
    void
    operator()(LHS& lhs, RHS& rhs, const std::array<len_type, NDim>& len) const
...
struct assign_expr_inner_loop_vec
{
    ...
    operator()(LHS& lhs, RHS& rhs, const std::array<len_type, NDim>& len) const
    {
        ...
        len_type misalignment =
            ((intptr_t)lhs.data % vector_traits<T>::alignment) / sizeof(T);
        ...
        len_type remainder = (len[Dim] - peel) % vector_traits<T>::vector_width;
        ...
        {
            lhs.template store_vec<NDim, Dim, Width, true>(
                i,
...
template <int NDim, int Dim = 1> struct assign_expr_loop_vec_row_major;

template <int NDim> struct assign_expr_loop_vec_row_major<NDim, NDim>
{
    template <typename LHS, typename RHS>
    void
    operator()(LHS& lhs, RHS& rhs, const std::array<len_type, NDim>& len) const
    {
        constexpr int Width =
            (vector_width<LHS>::value < vector_width<RHS>::value
                 ? vector_width<LHS>::value
                 : vector_width<RHS>::value);
        constexpr bool Aligned = false;

        assign_expr_inner_loop_vec<NDim, NDim - 1, Width, Aligned>()(lhs,
                                                                     rhs,
                                                                     len);
    }
};

template <int NDim, int Dim> struct assign_expr_loop_vec_row_major
{
    template <typename LHS, typename RHS>
    void
    operator()(LHS& lhs, RHS& rhs, const std::array<len_type, NDim>& len) const
...
template <int NDim, int Dim = NDim - 1> struct assign_expr_loop_vec_col_major;

template <int NDim> struct assign_expr_loop_vec_col_major<NDim, 0>
{
    template <typename LHS, typename RHS>
    void
    operator()(LHS& lhs, RHS& rhs, const std::array<len_type, NDim>& len) const
...
template <int NDim, int Dim> struct assign_expr_loop_vec_col_major
{
    template <typename LHS, typename RHS>
    void
    operator()(LHS& lhs, RHS& rhs, const std::array<len_type, NDim>& len) const
...
assign_expr(Array&& array_, Expr&& expr_)
{
    ...
    static_assert(
        expr_dimension<array_type>::value >= expr_dimension<expr_type>::value,
    ...
    else if (is_contiguous<NDim, 0>(array) && is_contiguous<NDim, 0>(expr))
    {
...
marray<Type, NDim, Allocator>::marray(const Expression& other)
{
    ...
    static_assert(
        NDim == expr_dimension<expr_type>::value,
