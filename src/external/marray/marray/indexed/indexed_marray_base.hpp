...
template <typename Type> class indexed_marray_view;

template <typename Type, typename Allocator = std::allocator<Type>>
class indexed_marray;

template <typename Type, typename Derived, bool Owner> class indexed_marray_base
{
  ...
  protected:
    ...
    void copy_(const indexed_marray_base<U, D, O>& other) const
    {
        ...
        {
            auto it = make_iterator(dense_lengths(),
                                    dense_strides(),
                                    other.dense_strides());
  ...
  public:
    ...
    {
        MARRAY_ASSERT(DenseNDim == dense_dimension());
        for_each_index_<marray_view<ctype, DenseNDim>>(
            std::forward<Func>(f),
    ...
    {
        MARRAY_ASSERT(DenseNDim == dense_dimension());
        for_each_index_<marray_view<Type, DenseNDim>>(
            std::forward<Func>(f),
            std::make_integer_sequence<int, IdxNDim>{});
    }
    ...
    const len_vector& dense_lengths() const { return dense_len_; }
    ...
    const len_vector& indexed_lengths() const { return idx_len_; }
    ...
    const stride_vector& dense_strides() const { return dense_stride_; }

    auto dimension() const { return dense_dimension() + indexed_dimension(); }

    int dense_dimension() const { return dense_len_.size(); }

    int indexed_dimension() const { return idx_.length(1); }
};
