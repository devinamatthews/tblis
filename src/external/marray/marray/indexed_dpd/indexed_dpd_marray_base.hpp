...
template <typename Type> class indexed_dpd_marray_view;
  ...
  protected:
               ...
               dpd_layout layout = DEFAULT_LAYOUT)
    {
        ...
        reset(irrep,
              ...
              idx_irrep,
              idx,
              default_depth(layout, dense_ndim),
              layout.base());
    }
    ...
    template <typename View, typename Func> void for_each_index_(Func&& f) const
    {
    ...
    template <typename Tp, typename Func> void for_each_element_(Func&& f) const
    {
        ...
        for (auto i : range(num_indices()))
        {
            ...
            while (it1.next())
            {
                for (auto j : range(dense_ndim)) irreps[j] = it1.irrep(j);
                           ...
                           std::integer_sequence<int, J...>) const
    {
        ...
        for (len_type i = 0; i < num_indices(); i++)
        {
            ...
            while (it1.next())
            {
                for (auto j : range(DenseNDim)) irreps[j] = it1.irrep(j);
  ...
  public:
    ...
    template <typename U,
              ...
              typename = std::enable_if_t<
                  !O_ && std::is_assignable<reference, U>::value>>
    const Derived&
    operator=(const indexed_dpd_marray_base<U, D, O>& other) const
    {
        copy_(other);
    ...
    indexed_dpd_marray_view<const Type> cview() const { return *this; }

    indexed_dpd_marray_view<ctype> view() const { return *this; }

    indexed_dpd_marray_view<Type> view() { return *this; }

    friend indexed_dpd_marray_view<const Type>
    cview(const indexed_dpd_marray_base& x)
    {
        return x;
    }
    ...
    {
        MARRAY_ASSERT(DenseNDim == dense_dimension());
        for_each_index_<dpd_marray_view<ctype>>(
            std::forward<Func>(f),
    ...
    {
        MARRAY_ASSERT(DenseNDim == dense_dimension());
        for_each_index_<dpd_marray_view<Type>>(
            std::forward<Func>(f),
            std::make_integer_sequence<int, IdxNDim>{});
    }
    ...
    matrix_view<const len_type> indices() const { return idx_; }
    ...
    len_vector indexed_lengths() const
    {
        auto ndim = indexed_dimension();
        len_vector len(ndim);
        for (auto dim : range(ndim)) len[dim] = idx_len_[dim][idx_irrep_[dim]];
        return len;
    }
    ...
    auto& indexed_irreps() const { return idx_irrep_; }

    auto irrep() const { return irrep_; }

    auto dense_irrep() const { return dense_irrep_; }

    auto num_irreps() const { return nirrep_; }

    auto num_indices() const { return std::max<len_type>(1, idx_.length(0)); }

    auto& permutation() const { return perm_; }

    auto dimension() const { return dense_dimension() + indexed_dimension(); }

    int dense_dimension() const { return perm_.size(); }

    int indexed_dimension() const { return idx_irrep_.size(); }
};
