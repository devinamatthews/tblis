...
inline stride_vector strides(const array_1d<len_type>& len,
                             layout layout = DEFAULT_LAYOUT)
{
    ...
    {
        stride[0] = 1;
        for (auto i : range(1, ndim)) stride[i] = stride[i - 1] * len_[i - 1];
    }
...
{
    struct no_layout : layout
    {
        constexpr no_layout() : layout(-1, construct{}) {}
    };
    ...
    detail::array_type_t<stride_type, NDim>
    stride(const detail::array_type_t<len_type, NDim>& len) const
    {
...
{
    struct no_base : index_base
    {
        constexpr no_base() : index_base(-1, construct{}) {}
    };
              ...
              detail::array_type_t<len_type, NDim>& base) const
    {
        ...
        else
        {
            if constexpr (NDim == DYNAMIC)
                base.resize(len.size());
            std::fill(base.begin(), base.end(), base_ == BASE_ZERO ? 0 : 1);
        }
    }

    detail::array_type_t<len_type, NDim>
    base(const detail::array_type_t<len_type, NDim>& len) const
    {
...
class marray_base
{
    static_assert(NDim > 0 || NDim == DYNAMIC,
                  "NDim must be positive or DYNAMIC");
  ...
  public:
    ...
    /** The type of a pointer to a tensor element. Maybe mutable or immutable
     * depending on the const-qualification of `Type`. */
    typedef Type* pointer;
    /** The type of an immutable pointer to a tensor element. */
    typedef const Type* const_pointer;
    /** The type of a reference to a tensor element. Maybe mutable or immutable
     * depending on the const-qualification of `Type`. */
    ...
    typedef
        typename detail::initializer_type<Type, NDim>::type initializer_type;
    typedef std::
        conditional_t<NDim == DYNAMIC, void, marray_iterator<marray_base>>
            iterator;
    typedef std::
        conditional_t<NDim == DYNAMIC, void, marray_iterator<const marray_base>>
            const_iterator;
    typedef std::
        conditional_t<NDim == DYNAMIC, void, std::reverse_iterator<iterator>>
            reverse_iterator;
    typedef std::conditional_t<NDim == DYNAMIC,
                               void,
  ...
  protected:
    ...
    {
        /*
         * The bounding box may have already been set by inherit_bbox_. If so,
        ...
        for (auto& s : bbox_stride_) s = std::max(std::abs(s), stride_type(1));
    }
...
#if !MARRAY_DOXYGEN
    std::enable_if_t<(NDim == 1 || NDim == DYNAMIC)
                         && std::is_convertible_v<T*, pointer>,
                     Derived&>
#else
    marray_view&
#endif
    ...
    /**
     * Reset to a view that wraps a raw data pointer, using the provided shape,
     ...
     *              [len_type](@ref MArray::len_type), including initializer
     *              lists.
     *
     ...
     *              Information specifying the memory layout and indexing for
     *              this view. May be one of the following:
     *                -# Nothing. The default layout and indexing are used.
     *                -# A pre-defined index base, layout, or combined
     *                   base/layout specifier: one of [BASE_ZERO](@ref
     *                   MArray::BASE_ZERO), [BASE_ONE](@ref MArray::BASE_ONE),
     *                   [ROW_MAJOR](@ref MArray::ROW_MAJOR),
     *                   [COLUMN_MAJOR](@ref MArray::COLUMN_MAJOR), [C](@ref
     *                   MArray::C), [CXX](@ref MArray::CXX), [FORTRAN](@ref
     *                   MArray::FORTRAN), or [MATLAB](@ref MArray::MATLAB).
     *                -# The strides of the tensor dimensions. May be any
     *                   one-dimensional container type whose elements are
     *                   convertible to [stride_type](@ref MArray::stride_type),
     *                   including initializer lists.
     *                -# The index base for each tensor dimension, followed by
     *                   the strides of each dimension. May be any combination
     *                   of either pre-defined index bases (e.g.
     *                   [BASE_ZERO](@ref MArray::BASE_ZERO)), pre-defined
     *                   layout (e.g. [ROW_MAJOR](@ref MArray::ROW_MAJOR)),
     *                   combined base/layout specifier (e.g. [FORTRAN](@ref
     *                   MArray::FORTRAN)), or one-dimensional containers whose
     *                   elements are convertible to [len_type](@ref
     *                   MArray::len_type) (index bases) or [stride_type](@ref
     *                   MArray::stride_type) (strides), including initializer
     *                   lists.
     *
     *              Examples:
     *                    - `1: view.reset(len, ptr) #all defaults are used`
     *                    - `2: view.reset(len, ptr, BASE_ONE) #default layout`
     *                    - `2: view.reset(len, ptr, COLUMN_MAJOR) #default
     *                      index base`
     *                    - `2: view.reset(len, ptr, FORTRAN)`
     *                    - `3: view.reset(len, ptr, {12, 1, 6}) #strides are
     *                      specified explicitly, default base`
     *                    - `4: view.reset(len, ptr, BASE_ONE, ROW_MAJOR)`
     *                    - `4: view.reset(len, ptr, BASE_ZERO, {1, 120, 40,
     *                      10})`
     *                    - `4: view.reset(len, ptr, {0, 0, 10}, COLUMN_MAJOR)`
     *                    - `4: view.reset(len, ptr, {-1, 0, 2}, {144, 12, 1})`
     *                    - `4: view.reset(len, ptr, FORTRAN,
     *                      std_vector_of_strides)`
                   ...
                   const layout_like& stride)
    {
        ...
        for (auto i : range(dimension())) MARRAY_ASSERT(len_[i] >= 0);
    ...
    /**
     * Reset to a view that wraps a raw data pointer, using the provided extents
     * and the specified layout.
     *
     * @note This overload provides functionality similar to the
     ...
     *       FORTRAN statement. This is by design such that the basic overloads
     *       with only `len` are equivalent to `begin = [0,...]` and `end =
     *       len`.
     *
     ...
     *              const-qualified.
     *
     * @param stride    The strides along each dimension, or one of
     *                  [ROW_MAJOR](@ref MArray::ROW_MAJOR), [COLUMN_MAJOR](@ref
     *                  MArray::COLUMN_MAJOR), [C](@ref MArray::C), [CXX](@ref
     *                  MArray::CXX), [FORTRAN](@ref MArray::FORTRAN),
     *                  [MATLAB](@ref MArray::MATLAB). If not specified, the
     *                  default layout is used.
     *
     * @return      A reference to the view.
     */
#if MARRAY_DOXYGEN
    marray_view& reset(indices begin,
                       indices end,
                       pointer ptr,
                       layout_or_strides stride = DEFAULT_LAYOUT);
#else
                   ...
                   const layout_like& stride)
    {
        ...
        for (auto i : range(len.size())) len[i] -= base[i];
    ...
    template <typename Ptr, typename Func> void for_each_element_(Func& f) const
    {
        auto it = make_iterator(lengths(), strides());
        Ptr ptr = const_cast<Ptr>(data());
    ...
    void copy_(const marray_base<U, N, D, O>& other) const
    {
        ...
        MARRAY_ASSERT(std::equal(lengths().begin(),
                                 lengths().end(),
                                 other.lengths().begin()));
  ...
  public:
    ...
    /**
     * Return the strides for a hypothetical tensor with the given lengths and
     * layout.
     *
     * @param len       The lengths of the hypothetical tensor.
     *
     * @param layout    The layout to use, either [ROW_MAJOR](@ref
     * MArray::ROW_MAJOR) or [COLUMN_MAJOR](@ref MArray::COLUMN_MAJOR). If
     ...
     * @param len       The lengths of the hypothetical tensor.
     *
     * @return          The number of elements, which is equal to the product of
     * the lengths.
     */
    ...
    /**
     * Return whether or not a tensor with the given lengths and strides would
     * have contiguous storage.
     *
     * @param len       The lengths of the hypothetical tensor.
     *
     * @param stride    The strides of the hypothetical tensor.
     *
     * @return          A `std::pair`, whose first member is a boolean value
     * indicating contiguous storage, and whose second member is the size of the
     * contiguous storage, or 0.
     */
    static std::pair<bool, stride_type>
    is_contiguous(const array_1d<len_type>& len,
     ...
     *              equal to the number of dimensions, and the supplied
     *              initializer lists must be "dense", i.e. every element must
     *              be specified.
     *
     ...
     *                  single tensor or tensor view, or a scalar. The
     *                  dimensions of the expression or tensor must either match
     *                  those of this tensor or be broadcast-compatible.
     *
     * @return          *this
     */
#if MARRAY_DOXYGEN
    ...
    std::enable_if_t<NDim != DYNAMIC && is_expression_arg<Expression>::value,
    ...
    /* Inherit docs */
    template <typename U, int N, typename D, bool O>
    std::enable_if_t<NDim == DYNAMIC || N == DYNAMIC || NDim == N,
                     const Derived&>
    ...
    /**
     * Perform an element-wise multiplication by the result of the specified
     * expression.
     *
     * For a tensor ([marray](@ref MArray::marray)), the instance must not be
     * const-qualified. For a tensor view ([marray_view](@ref
    ...
    /**
     * Perform an element-wise division by the result of the specified
     * expression.
     *
     * For a tensor ([marray](@ref MArray::marray)), the instance must not be
     * const-qualified. For a tensor view ([marray_view](@ref
     ...
     *
     * @tparam N The number of dimensions in the view, or [DYNAMIC](@ref
     *           MArray::DYNAMIC). The default is `NDim`.
     *
    ...
    /**
     * Return a possibly-const-qualified iterator to the first leading face or
     * element.
     *
     * The returned iterator selects leading faces of the tensor, i.e. a
     * subtensor formed by specifying the value of the first index. For a
     * one-dimensional tensor (vector), the iterator selects vector elements.
     * The iterator supports random access. The iterator is const-qualified if
     * the object is a tensor ([marray](@ref MArray::marray)), or a view
     * ([marray_view](@ref MArray::marray_view)) with a const-qualified value
     * type.
     *
     * @param dim The dimension along which to iterator. Default is the leading
     *            dimension.
     *
     * @returns A possibly-const-qualified iterator over the leading faces of
     *          the tensor.
     */
     ...
     * one-dimensional tensor (vector), the iterator selects vector elements.
     * The iterator supports random access. The iterator is const-qualified if
     * the object is a const-qualitied tensor ([marray](@ref MArray::marray)),
     * or a view ([marray_view](@ref MArray::marray_view)) with a
    ...
    /**
     * Return a possibly-const-qualified iterator to one past the last leading
     * face or element.
     *
     * The one-past-the-end iterator should not be dereferenced unless it is
     * decremented to a valid position. The returned iterator selects leading
     * faces of the tensor, i.e. a subtensor formed by specifying the value of
     * the first index. For a one-dimensional tensor (vector), the iterator
     * selects vector elements. The iterator supports random access. The
     * iterator is const-qualified if the object is a tensor
     * ([marray](@ref MArray::marray)), or a view ([marray_view](@ref
     * MArray::marray_view)) with a const-qualified value type.
     *
     * @param dim The dimension along which to iterator. Default is the leading
     *            dimension.
     *
     ...
     * the first index. For a one-dimensional tensor (vector), the iterator
     * selects vector elements. The iterator supports random access. The
     * iterator is const-qualified if the object is a const-qualified tensor
     * ([marray](@ref MArray::marray)), or a view ([marray_view](@ref
     * MArray::marray_view)) with a const-qualified value type.
     *
     * @param dim The dimension along which to iterator. Default is the leading
     ...
     * face or element.
     *
     * The returned iterator selects leading faces of the tensor in reverse
     * order, i.e. a subtensor formed by specifying the value of the first
     * index. For a one-dimensional tensor (vector), the iterator selects vector
     * elements. The iterator supports random access. The iterator is
     * const-qualified if the object is a tensor ([marray](@ref
     * MArray::marray)), or a view ([marray_view](@ref MArray::marray_view))
     * with a const-qualified value type.
     *
     * @param dim The dimension along which to iterator. Default is the leading
     *            dimension.
     *
     * @returns A possibly-const-qualified reverse iterator over the leading
     *          faces of the tensor.
     */
     ...
     * index. For a one-dimensional tensor (vector), the iterator selects vector
     * elements. The iterator supports random access. The iterator is
     * const-qualified if the object is a const-qualified tensor ([marray](@ref
     * MArray::marray)), or a view ([marray_view](@ref MArray::marray_view))
     * with a const-qualified value type.
     *
     * @param dim The dimension along which to iterator. Default is the leading
     ...
     * first leading face or element.
     *
     * The one-past-the-end iterator should not be dereferenced unless it is
     * decremented to a valid position. The returned iterator selects leading
     * faces of the tensor in reverse order, i.e. a subtensor formed by
     * specifying the value of the first index. For a one-dimensional tensor
     * (vector), the iterator selects vector elements. The iterator supports
     * random access. The iterator is const-qualified if the object is a tensor
     * ([marray](@ref MArray::marray)), or a view ([marray_view](@ref
     * MArray::marray_view)) with a const-qualified value type.
     *
     * @param dim The dimension along which to iterator. Default is the leading
     ...
     * specifying the value of the first index. For a one-dimensional tensor
     * (vector), the iterator selects vector elements. The iterator supports
     * random access. The iterator is const-qualified if the object is a
     * const-qualified tensor ([marray](@ref MArray::marray)), or a view
     * ([marray_view](@ref MArray::marray_view)) with a const-qualified value
     * type.
     *
     * @param dim The dimension along which to iterator. Default is the leading
     ...
     * `i+n[k]` in the original tensor or tensor view.
     *
     ...
     *
     * @param new_base   The base of the new view. Either a container type with
     * elements convertible to [len_type](@ref MArray::len_type) (including
     * initializer lists), or one of the tokens [BASE_ZERO](@ref
     * MArray::BASE_ZERO), [BASE_ONE](@ref MArray::BASE_ONE), [FORTRAN](@ref
     * MArray::FORTRAN), or [MATLAB](@ref MArray::MATLAB).
     *
     * @return      A possibly-mutable tensor view. For a tensor
     *              ([marray](@ref MArray::marray)), the returned view is
     ...
     *
     * @param new_base   The new base, either a container with elements
     *                   convertible to [len_type](@ref MArray::len_type) or one
     *                   of the tokens [BASE_ZERO](@ref MArray::BASE_ZERO),
     *                   [BASE_ONE](@ref MArray::BASE_ONE), [FORTRAN](@ref
     *                   MArray::FORTRAN), or [MATLAB](@ref MArray::MATLAB).
     */
    void rebase(const base_like& new_base)
    {
        ...
        for (auto i : range(dimension())) rebase(i, base[i]);
    }
     ...
     *              one-dimensional container type whose elements are
     *              convertible to `int`, including initializer lists. The
     *              values must form a permutation of `[0,N)`, where `N` is the
    ...
    std::enable_if_t<detail::are_convertible<int, Perm...>::value,
                     marray_view<Type, NDim>> permuted(const Perm&... perm)
    {
        marray_view<Type, NDim> r(*this);
        r.permute(perm...);
        return r;
    ...
    /**
     * Return a transposed view, that is, a view with the order of dimensions
     * reversed.
     *
     * @return      A possibly-mutable tensor view. For a tensor
     *              ([marray](@ref MArray::marray)), the returned view is
    ...
    marray_view<Type, NDim> transposed()
    {
        if constexpr (NDim != DYNAMIC)
        {
            ...
            for (auto i : range(NDim)) dims[i] = NDim - i - 1;
...
#else
    marray_view<Type, NDim> T() { return transposed(); }
     ...
     *
     * @tparam NewNDim  The number of dimensions in the lowered view or
     *                  [DYNAMIC](@ref MArray::DYNAMIC). If `split` is a
     *                  comma-separated list of split points (e.g. `t.split(1,
     *                  3, 4)`), then `NewNDim` is deduced as the number of
     *                  split points plus one.
     *
     ...
     *              increasing order and in the range `[1,N)`. May be any
     *              one-dimensional container type whose elements are
     *              convertible to `int`, including initializer lists, or a
     *              comma-separated list of split points (i.e. multiple
     *              arguments of any integral type, one for each split point).
     *
     * @return      A possibly-mutable tensor view. For a tensor
     *              ([marray](@ref MArray::marray)), the returned view is
     *              mutable if the instance is not const-qualified.
    ...
    marray_view<Type, NewNDim> lowered(const array_1d<int>& split)
    {
        ...
        for (auto i : range(nsplit))
        {
            ...
            if (i != 0)
                MARRAY_ASSERT(split_[i - 1] < split_[i]);
        }
     ...
     *
     * @tparam NewNDim  The number of dimensions in the reshaped view or
     *                  [DYNAMIC](@ref MArray::DYNAMIC). If `lengths` is a
     *                  comma-separated list of lengths (e.g.
     *                  `t.reshaped(1, 3, 4)`), then `NewNDim` is deduced as the
     *                  number of lengths provided.
     *
    ...
    marray_view<Type, NewNDim> reshaped(const array_1d<len_type>& lengths_)
    {
        ...
        if constexpr (NewNDim == DYNAMIC)
            new_strides.resize(new_dimension);

        for (auto l : new_lengths) MARRAY_ASSERT(l >= 0);
        ...
        for (auto l : new_lengths)
            if (l == 0)
            {
                ...
                return marray_view<Type, NewNDim>{new_lengths,
                                                  nullptr,
                                                  new_strides};
            }
        ...
        auto relative_order = [&](auto i, auto j)
        { return old_strides[i] < old_strides[j] ? COLUMN_MAJOR : ROW_MAJOR; };
             ...
             old_begin = old_end, new_begin = new_end)
        {
            ...
            len_type size = 1;
            for (auto i : range(old_begin, old_end)) size *= old_lengths[i];
     ...
     * `n-1-i` in the original tensor or view, where `n` is the tensor length
    ...
    /**
     * Return a view where the order of the indices along the given dimension
     * has been reversed.
     *
     * For the indicated dimension, an index of `i` in the reversed tensor
     * corresponds to an index of `n-1-i` in the original tensor, where `n` is
     * the tensor length along that dimension.
     *
    ...
    /**
     * Return an immutable reference or view to an element or face along the
     * specified dimension.
     *
     * If the tensor has more than one dimension, a tensor face is returned.
     * Otherwise, a reference to an element is returned.
     *
     * @param dim   The dimension along which to extract the reference or face.
     *              Default is the leading dimension.
     *
    ...
    /**
     * Return a reference or view to an element or face along the specified
     * dimension.
     *
     ...
     * @param i     The index of the element or face to extract.
     *
     * @return  For a tensor ([marray](@ref MArray::marray)), the returned
     *          reference or view is mutable if the instance is not
     *          const-qualified. For a tensor view ([marray_view](@ref
     *          MArray::marray_view)), the returned reference or view is mutable
    ...
    template <typename = void, int N = NDim>
    std::enable_if_t<N != 1,
                     marray_view<Type, NDim == DYNAMIC ? DYNAMIC : NDim - 1>>
    ...
    /* Inherit docs */
    template <typename = void, int N = NDim>
    std::enable_if_t<N == 1, reference> slice(int dim, len_type i)
    {
        (void)dim;
        MARRAY_ASSERT(dim == 0);
        return (*this)[i];
    ...
    /**
     * Return an immutable reference or view to the first element or face along
    ...
    /**
     * Return a reference or view to the first element or face along the
     * specified dimension.
     *
     * @param dim   The dimension along which to extract the element or face.
     *              Default is the leading dimension.
    ...
    /* Inherit docs */
    template <typename = void, int N = NDim>
    std::enable_if_t<N == 1, reference> front(int dim = 0)
    {
        (void)dim;
        MARRAY_ASSERT(dim == 0);
        return (*this)[base()];
    ...
    /**
     * Return an immutable reference or view to the last element or face along
     * the specified dimension.
     *
     * If the tensor has more than one dimension, a tensor face is returned.
     * Otherwise, a reference to an element is returned.
    ...
    /**
     * Return a reference or view to the last element or face along the
     * specified dimension.
     *
     * @param dim   The dimension along which to extract the element or face.
     *              Default is the leading dimension.
    ...
    /* Inherit docs */
    template <typename = void, int N = NDim>
    std::enable_if_t<N == 1, reference> back(int dim = 0)
    {
        (void)dim;
        MARRAY_ASSERT(dim == 0);
     ...
     * the final view or reference is mutable if the instance is not
     * const-qualified. For a tensor view ([marray_view](@ref
     * MArray::marray_view)), the final view or reference is mutable if the
     * value type is not const-qualified.
     *
     * @note Only available when `NDim != ` [DYNAMIC](@ref MArray::DYNAMIC).
     *       Otherwise, use [operator()](@ref operator()(const
     *       array_1d<len_type>&)).
     *
...
#else
    template <typename = void, int N = NDim>
    std::enable_if_t<N == 1, reference> operator[](len_type i)
    {
        i -= base();
     ...
     * combination.
     *
     * This overload specifies a range along a dimension, and does not reduce
     ...
     *
     * @note Only available when `NDim != ` [DYNAMIC](@ref MArray::DYNAMIC).
     *       Otherwise, use @ref operator()().
     *
     * @param x     The specified range (either the result of [range](@ref
     *              MArray::range) or [all](@ref MArray::slice::all)). The
     *              dimension to which this range refers depends on how many []
     *              operators have been applied. The first [] refers to the
     *              first dimension and so on.
     *
     * @return      A temporary indexing object which can be converted to a
     *              tensor view.
     */
#if MARRAY_DOXYGEN
     ...
     * combination.
     *
     * This overload specifies that the tensor data should be repeated some
     * number of times. The indexing object returned by this overload cannot be
     * converted to a view or used on the left-hand side of an assignment, but
     * it can be used in the right-hand side of any tensor expression.
     * Explicitly broadcasting allows the dimensions of different tensors in the
     * expression to be lined up as desired.
     *
     * @note Only available when `NDim != ` [DYNAMIC](@ref MArray::DYNAMIC).
     *       Otherwise, use @ref operator()().
     *
     ...
     *
     * @param idx   The indices of the desired element. The number of indices
     *              must be equal to the number of dimensions. May be any
     *              one-dimensional container type whose elements are
     *              convertible to a tensor length, including initializer lists.
     *
     * @return  For a tensor ([marray](@ref MArray::marray)), the returned
     *          reference is mutable if the instance is not const-qualified. For
     *          a tensor view ([marray_view](@ref MArray::marray_view)), the
     *          returned reference is mutable if the value type is not
     *          const-qualified.
     */
#if MARRAY_DOXYGEN
     ...
     * `tensor[arg1][arg2][arg2]...`, except that it is also applicable to
     * tensors and views with `NDim == ` [DYNAMIC](@ref MArray::DYNAMIC). In
     * that case, the result is the same as
     * `tensor.view<N>()[arg1][arg2][arg3]...` where `N` is the number of
     * non-broadcast arguments, and must be the same as the number of tensor
     * dimensions.
     *
     * For a tensor ([marray](@ref MArray::marray)),
     * the final view or reference is mutable if the instance is not
     ...
     * value type is not const-qualified.
     *
     * @param idx  Parameter pack of indices, ranges, [all](@ref
     *             MArray::slice::all), or [bcast](@ref MArray::slice::bcast).
     *
     * @return      If all indices have been explicitly specified, a reference
     *              to the indicated tensor element. Otherwise, a temporary
     *              indexing object which can be converted to a tensor view
     *              (unless one or more broadcast dimensions have been
     *              introduced).
     */
#if MARRAY_DOXYGEN
     ...
     *
     * @tparam N  The number of dimensions which will be iterated over, or
     * [DYNAMIC](@ref MArray::DYNAMIC). If `NDim` and `N` are both not
     * [DYNAMIC](@ref MArray::DYNAMIC), then they must be equal.
     *
     * @param f
     * @parblock  A function or functor callable as either `f(e)`, where `e` is
     *            a reference to a tensor element, or
     *
     *            - if `N != ` [DYNAMIC](@ref MArray::DYNAMIC) : `f(e, i0, i1,
     *              ...)` where `i0, i1, ...` are indices, one for each
     *              dimension, or
     *            - if `N == ` [DYNAMIC](@ref MArray::DYNAMIC) : `f(e, idx)`
     *              where `idx` is an unspecified container type of indices, one
     *              for each dimension.
     *
     *            For a tensor ([marray](@ref MArray::marray)),
     *            the elements are mutable if the instance is not
     *            const-qualified. For a tensor view ([marray_view](@ref
     *            MArray::marray_view)), the elements are mutable if the value
    ...
    template <int N = NDim, typename Func> void for_each_element(Func&& f) const
    {
        static_assert(N == DYNAMIC || NDim == DYNAMIC || N == NDim);
     ...
     *         all zero indices. In general this pointer should not be
     *         dereferenced unless is has been offset by the product of the
     *         tensor strides with a valid set of indices.
     */
#if MARRAY_DOXYGEN
    immutable_pointer
#else
     ...
     *         all zero indices. In general this pointer should not be
     *         dereferenced unless is has been offset by the product of the
     *         tensor strides with a valid set of indices. For a tensor
     *         ([marray](@ref MArray::marray)), the returned pointer is
     *         immutable if the instance is const-qualified. For a tensor view
     *         ([marray_view](@ref MArray::marray_view)), the returned pointer
     *         is immutable if the value type is const-qualified.
     */
#if MARRAY_DOXYGEN
    possibly_immutable_pointer origin();
#else
    pointer origin() { return const_cast<marray_base&>(*this).origin(); }

    /* Inherit docs */
    cptr origin() const
    {
        auto ptr = data();
        for (auto i : range(dimension())) ptr -= base(i) * stride(i);
        return ptr;
    }
#endif
     ...
     *
     * @return An immutable pointer that points to the element with `index ==
     * base`.
     */
#if MARRAY_DOXYGEN
    immutable_pointer
     ...
     *         For a tensor ([marray](@ref MArray::marray)), the returned
     *         pointer is immutable if the instance is const-qualified. For a
     *         tensor view ([marray_view](@ref MArray::marray_view)), the
     *         returned pointer is immutable if the value type is
     *         const-qualified.
     */
#if MARRAY_DOXYGEN
    ...
    /* Inherit docs */
    cptr data() const { return const_cast<marray_base&>(*this).data(); }
#endif
     ...
     */
    auto& lengths() const { return len_; }
     ...
     */
    auto& strides() const { return stride_; }

    /**
     * Return the number of dimensions.
     *
     * @note The return value is only truly a constant expresson if `N` is not
     *       [DYNAMIC](@ref MArray::DYNAMIC).
     *
     * @return The number of dimensions.
     */
    constexpr int dimension() const { return int(lengths().size()); }
 ...
 *
 * @tparam N The number of dimensions in the resulting view or [DYNAMIC](@ref
 *           MArray::DYNAMIC). The default is the same number of dimensions in
 *           the tensor (if fixed), or [DYNAMIC](@ref MArray::DYNAMIC) (if not).
 *
 * @return  An immutable view.
 *
 * @ingroup funcs
 */
#if MARRAY_DOXYGEN
template <int N> immutable_view cview(tensor_or_view x);
#else
template <typename Type, int NDim, typename Derived, bool Owner>
 ...
 *           the tensor (if fixed), or [DYNAMIC](@ref MArray::DYNAMIC) (if not).
 *
 * @return  A possibly-mutable tensor view. For a tensor ([marray](@ref
 *          MArray::marray)), the returned view is immutable if the instance is
 *          const-qualified. For a tensor view ([marray_view](@ref
 *          MArray::marray_view)), the returned view is mutable if the value
 *          type is not const-qualified.
 *
 * @ingroup funcs
 */
#if MARRAY_DOXYGEN
template <int N> possibly_mutable_view view(tensor_or_view x);
#else
template <typename Type, int NDim, typename Derived, bool Owner>
...
/**
 * Write out a representation of the tensor or tensor view to the given output
 * stream. The format resembles the following (i.e., a valid C/C++ nested array
 * initializer):
 *
 * @code
 * {
...
std::ostream& operator<<(std::ostream& os,
                         const marray_base<Type, NDim, Derived, Owner>& x)
#endif
{
    auto N = x.dimension();

    for (auto i : range(N - 1)) os << std::string(i, ' ') << "{\n";
    ...
    for (bool done = false; !done;)
    {
        ...
        if (n > 0)
        {
            for (auto i : range(n - 1)) os << data[i * stride[N - 1]] << ", ";
