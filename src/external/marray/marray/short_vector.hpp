...
class short_vector
{
  ...
  public:
    ...
    short_vector() : _size(0), _alloc(Allocator(), _local_data()) {}
    ...
    : _size(other._size),
      _alloc(std::move(other._alloc), _local_data())
    {
        if (other._is_local())
        {
    ...
    pointer data() { return _alloc._data; }

    const_pointer data() const { return _alloc._data; }
    ...
    size_type capacity() const { return _is_local() ? N : _capacity; }

    void shrink_to_fit() { _set_capacity(size()); }
    ...
    void push_back(const T& value) { _emplace(end(), 1, value); }
    ...
    void swap(short_vector& other)
    {
                ...
                != other._alloc))
        {
            std::swap_ranges(begin(),
                             begin() + std::min(size(), other.size()),
                             other.begin());
            if (size() < other.size())
            {
                _uninitialized_move_n_if(other.begin() + size(),
                                         other.size() - size(),
                                         end());
            ...
            {
                _uninitialized_move_n_if(begin() + other.size(),
                                         size() - other.size(),
                                         other.end());
    ...
    friend void swap(short_vector& lhs, short_vector& rhs) { lhs.swap(rhs); }
  ...
  protected:
    ...
    void _assign(Iterator first, Iterator last, std::input_iterator_tag)
    {
        ...
        for (; it != end() && first != last; ++it, ++first) *it = *first;
    ...
    template <typename Iterator> void _destroy(Iterator first, Iterator last)
    {
        if (first == last)
            return;
    ...
    iterator _construct_n(iterator result, size_type n, Args&&... args)
    {
        ...
        {
            for (; cur != last; ++cur)
                _alloc_traits::construct(_alloc,
                                         cur,
    ...
    iterator _emplace(const_iterator cpos, size_type n, Args&&... args)
    {
        ...
        _uninitialized_move_n_if(end() - n_uninit,
                                 n_uninit,
                                 end() + n_construct);
    ...
    template <typename Iterator>
    iterator _insert(const_iterator cpos,
                     Iterator first,
                     Iterator last,
    ...
    const_pointer _local_data() const { return &_fixed[0]; }

    bool _is_local() const { return _alloc._data == _local_data(); }

    size_type _size;

    struct _data_s : public Allocator
    {
        _data_s(Allocator alloc, pointer data) : Allocator(alloc), _data(data)
        {
        }
