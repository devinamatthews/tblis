...
struct any_container_base
{
    ...
    any_container_base(const std::type_info& type) : type(type) {}
...
template <typename T, typename = void> struct any_container;

template <typename T>
struct any_container<T, enable_if_t<std::is_scalar<T>::value>>
: any_container_base
{
    T data;

    any_container(T obj) : any_container_base(typeid(T)), data(obj) {}

    std::unique_ptr<any_container_base> clone() const
    {
        return std::unique_ptr<any_container_base>(new any_container(data));
    }
};

template <typename T>
struct any_container<T, enable_if_t<!std::is_scalar<T>::value>>
: T, any_container_base
{
    any_container(const T& obj) : any_container_base(typeid(T)), T(obj) {}

    any_container(T&& obj) : any_container_base(typeid(T)), T(std::move(obj)) {}
...
class any
{
  public:
    ...
    template <typename T,
              typename = enable_if_t<!is_same<decay_t<T>, any>::value>>
    any(T&& value)
    ...
    template <typename T> enable_if_t<std::is_scalar<T>::value, T&> get()
    {
        if (typeid(T) != type() || !data_)
            throw std::bad_cast();
