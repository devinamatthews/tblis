...
class bounded_vector
{
  ...
  public:
    ...
    explicit bounded_vector(size_type count) { resize(count); }

    template <typename InputIterator>
    bounded_vector(InputIterator first,
                   InputIterator last,
                   const Allocator& alloc = Allocator())
    : allocator_(alloc)
    {
    ...
    bounded_vector(const bounded_vector& other) : allocator_(other.allocator_)
    {
        for (const T& x : other) { push_back(x); }
    }

    bounded_vector(bounded_vector&& other) : allocator_(other.allocator_)
    {
    ...
    T* data() { return reinterpret_cast<T*>(allocator_.data_); }
    ...
    iterator insert(const_iterator pos_, const T& value)
    {
        ...
        alloc_traits_::construct(allocator_, data() + size_, std::move(back()));
        ++size_;
    ...
    template <
        ...
        typename = enable_if_t<is_convertible<
            ...
            std::input_iterator_tag>::value>>
    iterator
    insert(const_iterator pos_, InputIterator first, InputIterator last)
    {
        iterator pos = begin() + (pos_ - cbegin());
        insert_(
            pos,
    ...
    {
        alloc_traits_::construct(allocator_, data() + size_, std::move(value));
        ++size_;
    }

    template <typename... Args> void emplace_back(Args&&... args)
    {
        alloc_traits_::construct(allocator_,
                                 data() + size_,
  ...
  private:
    ...
    struct allocator_data : Allocator
    {
        ...
        allocator_data(const Allocator& alloc) : Allocator(alloc) {}
