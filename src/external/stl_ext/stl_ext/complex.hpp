...
template <typename T> struct real_type<complex<T>>
{
    typedef T type;
};
...
template <typename T> struct complex_type<complex<T>>
{
    typedef complex<T> type;
};
...
template <typename T> constexpr inline auto is_complex_v = is_complex<T>::value;

template <typename T> std::enable_if_t<is_complex_v<T>, T> conj(T x)
{
    return {x.real(), -x.imag()};
}

template <typename T> std::enable_if_t<std::is_arithmetic_v<T>, T> conj(T x)
{
    return x;
}

template <typename T> T conj(bool conjugate, T val)
{
...
template <typename T> std::enable_if_t<std::is_arithmetic_v<T>, T> norm2(T x)
{
    return x * x;
}
...
stl_ext::enable_if_t<
    is_arithmetic<T>::value && is_arithmetic<U>::value && !is_same<T, U>::value,
...
template <typename T, typename U>
stl_ext::enable_if_t<is_arithmetic<T>::value && is_arithmetic<U>::value, bool>
