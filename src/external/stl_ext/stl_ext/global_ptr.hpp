...
template <typename T> class global_ptr
{
  public:
    ...
    void swap(global_ptr& other) { ptr_.swap(other.ptr_); }

    friend void swap(global_ptr& p1, global_ptr& p2) { p1.swap(p2); }
    ...
    void reset(T* p) { ptr_ = std::make_shared<T*>(p); }
    ...
    const T* get() const { return ptr_ ? *ptr_ : nullptr; }
