...
template <typename list_> class ptr_list_
{
  private:
    template <typename ptr_> class iterator_
    {
      ...
      public:
        ...
        typedef typename std::iterator_traits<ptr_>::difference_type
            difference_type;
        ...
        typedef conditional_t<is_same<ptr_, value_type*>::value,
                              ...
                              typename list_::const_iterator>
            ptr_iterator;

        iterator_() = default;

        iterator_(const iterator_& other) : it_(other.it_) {}

        template <typename T = pointer, typename = enable_if_const_pointer_t<T>>
        iterator_(const iterator_<value_type*>& other) : it_(other.it_)
        {
        }
        ...
        iterator_ operator++(int x) { return iterator_(it_++); }
        ...
        ptr_iterator& base() { return it_; }

        const ptr_iterator& base() const { return it_; }
  ...
  public:
    ...
    ptr_list_(size_type n, const value_type& val) { assign(n, val); }
    ...
    ptr_list_(std::initializer_list<value_type> il) { assign(il); }

    ptr_list_(std::initializer_list<pointer> il) { assign(il); }
    ...
    ptr_iterator pbegin() { return impl_.begin(); }

    ptr_iterator pend() { return impl_.end(); }

    const_ptr_iterator pbegin() const { return impl_.begin(); }

    const_ptr_iterator pend() const { return impl_.end(); }

    reverse_ptr_iterator rpbegin() { return impl_.rbegin(); }

    reverse_ptr_iterator rpend() { return impl_.rend(); }

    const_reverse_ptr_iterator rpbegin() const { return impl_.rbegin(); }

    const_reverse_ptr_iterator rpend() const { return impl_.rend(); }

    const_ptr_iterator cpbegin() const { return impl_.begin(); }

    const_ptr_iterator cpend() const { return impl_.end(); }

    const_reverse_ptr_iterator crpbegin() const { return impl_.rbegin(); }

    const_reverse_ptr_iterator crpend() const { return impl_.rend(); }

    size_type size() const { return impl_.size(); }

    size_type max_size() const { return impl_.max_size(); }

    void resize(size_type n) { impl_.resize(n); }
    ...
    ptr_type& pfront() { return impl_.front(); }

    ptr_type& pback() { return impl_.back(); }

    const ptr_type& pfront() const { return impl_.front(); }

    const ptr_type& pback() const { return impl_.back(); }
    ...
    void assign(const ptr_list_& x) { *this = x; }

    void assign(ptr_list_&& x) { *this = std::move(x); }

    void assign(std::initializer_list<value_type> il)
    {
        impl_.clear();

        for (auto& val : il) { impl_.emplace_back(new value_type(val)); }
    }
    ...
    void assign(std::initializer_list<pointer> il)
    {
        ...
        for (auto& ptr : il) { impl_.emplace_back(ptr); }
    }
    ...
    void push_front(const ptr_type& x) { impl_.push_front(x); }

    void push_front(ptr_type&& x) { impl_.push_front(move(x)); }

    template <typename T = pointer,
              typename = enable_if_not_same_t<T, ptr_type>>
    ...
    void push_back(const ptr_type& x) { impl_.push_back(x); }

    void push_back(ptr_type&& x) { impl_.push_back(move(x)); }
    ...
    {
        typename list_::iterator i = ci2i(impl_, position.it_);
        for (auto& val : il) { impl_.emplace(i, new value_type(val)); }
    ...
    {
        typename list_::iterator i = ci2i(impl_, position.it_);
        for (auto& ptr : il) { impl_.emplace(i, ptr); }
        return prev(i, il.size());
    }
    ...
    void swap(ptr_list_& x) { impl_.swap(x.impl_); }
    ...
    {
        return impl_.emplace(ci2i(impl_, position.it_),
    ...
    friend void swap(ptr_list_&& a, ptr_list_&& b) { a.swap(b); }
    ...
    void splice(const_iterator position, ptr_list_&& x, const_iterator i)
    {
        impl_.splice(ci2i(impl_, position.it_),
                     std::move(x.impl_),
                     ci2i(x.impl_, i.it_));
    }
                ...
                const_iterator last)
    {
        impl_.splice(ci2i(impl_, position.it_),
                     x.impl_,
                     ci2i(x.impl_, first.it_),
                     ci2i(x.impl_, last.it_));
    }
    ...
    void unique()
    {
        impl_.unique([](const ptr_type& a, const ptr_type& b)
                     { return *a == *b; });
    }
    ...
    {
        impl_.merge(x.impl_,
                    [](const ptr_type& a, const ptr_type& b)
                    { return *a < *b; });
    }

    void merge(ptr_list_&& x)
    {
        impl_.merge(move(x.impl_),
                    [](const ptr_type& a, const ptr_type& b)
                    { return *a < *b; });
    }

    template <class Compare> void merge(ptr_list_& x, Compare comp)
    {
        impl_.merge(x.impl_,
                    [&comp](const ptr_type& a, const ptr_type& b)
                    { return comp(*a, *b); });
    }
    ...
    void reverse() noexcept { impl_.reverse(); }
...
template <typename T> using ptr_list = detail::ptr_list_<std::list<T*>>;
