...
template <typename vector_> class ptr_vector_
{
  private:
    template <typename ptr_> class iterator_
    {
      ...
      public:
        ...
        difference_type
        operator-(const iterator_<const value_type*>& other) const
        {
            return it_ - other.it_;
        }
  ...
  public:
    ...
    ptr_vector_(size_type n, const value_type& val) { assign(n, val); }
    ...
    ptr_vector_(std::initializer_list<value_type> il) { assign(il); }

    ptr_vector_(std::initializer_list<pointer> il) { assign(il); }
    ...
    reverse_iterator rbegin() { return reverse_iterator(impl_.end()); }

    reverse_iterator rend() { return reverse_iterator(impl_.begin()); }
    ...
    void shrink_to_fit() { impl_.shrink_to_fit(); }
    ...
    ptr_type& ptr(size_type n) { return impl_.at(n); }

    const ptr_type& ptr(size_type n) const { return impl_.at(n); }
    ...
    void assign(const ptr_vector_& x) { *this = x; }

    void assign(ptr_vector_&& x) { *this = std::move(x); }
    ...
    void push_back(ptr_type&& x) { impl_.push_back(std::move(x)); }
    ...
    void swap(ptr_vector_& x) { impl_.swap(x.impl_); }
    ...
    {
        return impl_.emplace(position.it_,
    ...
    friend void swap(ptr_vector_&& a, ptr_vector_&& b) { a.swap(b); }
...
template <typename T> using ptr_vector = detail::ptr_vector_<std::vector<T*>>;
