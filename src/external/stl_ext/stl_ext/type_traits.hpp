...
template <typename T, typename U, typename = void> struct common_type;
...
using enable_if_floating_point_t =
    typename enable_if_floating_point<T, U>::type;
template <typename T, typename U = void>
using enable_if_not_floating_point = enable_if<!is_floating_point<T>::value, U>;
template <typename T, typename U = void>
using enable_if_not_floating_point_t =
    typename enable_if_not_floating_point<T, U>::type;
...
using enable_if_not_arithmetic_t =
    typename enable_if_not_arithmetic<T, U>::type;

using std::is_pointer;

template <typename T> constexpr bool is_pointer_v()
{
...
template <typename T> constexpr bool is_reference_v()
{
...
template <typename T, typename U> constexpr bool is_convertible_v()
{
...
using enable_if_not_convertible_t =
    typename enable_if_not_convertible<T, U, V>::type;

using std::is_base_of;

template <typename T, typename U> constexpr bool is_base_of_v()
{
...
template <typename T>
struct is_const_pointer : is_const_pointer_helper<remove_cv_t<T>>
{
};

template <typename T> constexpr bool is_const_pointer_v()
{
    return is_const_pointer<T>::value;
}
...
: std::integral_constant<bool,
                         is_pointer<T>::value && !is_const_pointer<T>::value>
{
};

template <typename T> constexpr bool is_non_const_pointer_v()
{
    return is_const_pointer<T>::value;
}

template <typename T, typename U = void>
using enable_if_non_const_pointer =
    enable_if<is_non_const_pointer<T>::value, U>;
template <typename T, typename U = void>
using enable_if_non_const_pointer_t =
    typename enable_if_non_const_pointer<T, U>::type;

template <typename...> struct enable_if_exists
{
    typedef void type;
};
...
template <typename T, typename U>
using is_similar = is_same<remove_cv_t<T>, remove_cv_t<U>>;

template <typename T, typename U> constexpr bool is_similar_v()
{
    return is_similar<T, U>::value;
}
...
template <typename... Args>
using has_member =
    stl_ext::conditional_t<false, has_member_helper<Args...>, void>;
