...
template <size_t N, typename T, typename U, typename... Ts> struct vec_helper
{
...
template <typename T, typename U> struct vec_helper<1, T, U>
{
    vec_helper(vector<T>& v, U&& t) { v.push_back(std::forward<U>(t)); }
};

} // namespace detail

template <typename T, typename... Ts> vector<decay_t<T>> vec(T&& t, Ts&&... ts)
{
    vector<decay_t<T>> v;
    v.reserve(1 + sizeof...(Ts));
    detail::vec_helper<1 + sizeof...(Ts), decay_t<T>, T, Ts...>(
        v,
        std::forward<T>(t),
        std::forward<Ts>(ts)...);
    return v;
...
template <typename T> vector<T> slice(vector<T>&& v, ptrdiff_t e1, ptrdiff_t e2)
{
    if (e1 < 0)
        e1 += v.size();
    if (e2 < 0)
...
template <typename T> vector<T> slice(const vector<T>& v, ptrdiff_t e1)
{
    if (e1 < 0)
        e1 += v.size();
...
template <typename T> vector<T> slice(vector<T>&& v, ptrdiff_t e1)
{
    if (e1 < 0)
        e1 += v.size();
    return vector<T>(std::make_move_iterator(v.begin() + e1),
