...
template <int I> struct foo1<I, enable_if_t<(I >= 0)>>
{
    int x[1];
};
...
template <typename T> struct foo2<T, enable_if_same_t<T, int>>
{
    int x[1];
};

template <typename T> struct foo2<T, enable_if_not_same_t<T, int>>
{
    int x[2];
};
...
template <typename T> struct foo3<T, enable_if_const_t<T>>
{
    int x[1];
};

template <typename T> struct foo3<T, enable_if_non_const_t<T>>
{
    int x[2];
};
...
template <typename T> struct foo4<T, enable_if_integral_t<T>>
{
    int x[1];
};

template <typename T> struct foo4<T, enable_if_not_integral_t<T>>
{
    int x[2];
};
...
template <typename T> struct foo5<T, enable_if_floating_point_t<T>>
{
    int x[1];
};

template <typename T> struct foo5<T, enable_if_not_floating_point_t<T>>
{
    int x[2];
};
...
template <typename T> struct foo6<T, enable_if_arithmetic_t<T>>
{
    int x[1];
};

template <typename T> struct foo6<T, enable_if_not_arithmetic_t<T>>
{
    int x[2];
};
...
template <typename T> struct foo7<T, enable_if_pointer_t<T>>
{
    int x[1];
};

template <typename T> struct foo7<T, enable_if_not_pointer_t<T>>
{
    int x[2];
};
...
template <typename T> struct foo8<T, enable_if_reference_t<T>>
{
    int x[1];
};

template <typename T> struct foo8<T, enable_if_not_reference_t<T>>
{
    int x[2];
};
...
template <typename T> struct foo9<T, enable_if_convertible_t<T, int>>
{
    int x[1];
};

template <typename T> struct foo9<T, enable_if_not_convertible_t<T, int>>
{
    int x[2];
};
...
template <typename T> struct foo10<T, enable_if_base_of_t<bar10, T>>
{
    int x[1];
};

template <typename T> struct foo10<T, enable_if_not_base_of_t<bar10, T>>
{
    int x[2];
};
...
template <typename T, typename = void> struct foo11
{
    int x[2];
};

template <typename T> struct foo11<T, enable_if_exists_t<typename T::type>>
{
    int x[1];
};
...
template <typename T> struct foo12<T, enable_if_similar_t<T, int>>
{
    int x[1];
};

template <typename T> struct foo12<T, enable_if_not_similar_t<T, int>>
{
    int x[2];
};
