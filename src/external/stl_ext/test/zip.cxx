...
TEST(unit_zip, cbegin)
{
    ...
    EXPECT_EQ(
        make_tuple(get<0>(v).cbegin(), get<1>(v).cbegin(), get<2>(v).cbegin()),
        i);
}
...
template <typename T, T... S> struct print_integer_sequence;

template <typename T> struct print_integer_sequence<T>
{
    print_integer_sequence(ostream& os) { os << "(null)"; }
};

template <typename T, T S1> struct print_integer_sequence<T, S1>
{
    print_integer_sequence(ostream& os) { os << S1; }
};

template <typename T, T S1, T... S> struct print_integer_sequence<T, S1, S...>
{
...
TEST(unit_zip, concat_sequences)
{
    EXPECT_TRUE((is_same<std::integer_sequence<size_t>,
                         typename detail::concat_sequences<
                             size_t,
    ...
    EXPECT_TRUE((is_same<std::integer_sequence<size_t, 0, 1>,
                         typename detail::concat_sequences<
                             ...
                             std::integer_sequence<size_t, 0>>::type>::value));
    EXPECT_TRUE(
        (is_same<std::integer_sequence<size_t, 0, 1, 2, 3, 4, 5>,
                 typename detail::concat_sequences<
                     size_t,
...
{
    call(
        [](int x, unique_ptr<int>&& y, const vector<int>& z)
        {
            EXPECT_EQ(3, x);
            EXPECT_EQ(1, *y);
