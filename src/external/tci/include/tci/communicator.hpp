...
{
  protected:
    template <typename Func, typename... Args> struct broadcast_from_internal
    {
        template <size_t... I>
        broadcast_from_internal(const communicator& comm,
                                unsigned root,
                                Func&& func,
  ...
  public:
    class deferred_task_set
    {
      ...
      public:
        ~deferred_task_set() { tci_task_set_destroy(&_tasks); }

        template <typename Func> void visit(unsigned task, Func&& func)
        {
      ...
      protected:
        ...
        template <typename Func> void visit_all(Func&& func)
        {
            tci_task_set_visit_all(
                &_tasks,
                [](tci_comm* comm, unsigned task, void* payload)
                {
                    (*(typename std::decay<Func>::type*)payload)(
                        *reinterpret_cast<const communicator*>(comm),
                        task);
                },
                &func);
        }
    ...
    communicator() { tci_comm_init_single(*this); }

    ~communicator() { tci_comm_destroy(*this); }

    communicator(const communicator&) = delete;

    communicator(communicator&& other) : _comm(other._comm)
    {
    ...
    bool master() const { return tci_comm_is_master(*this); }
    ...
    unsigned num_threads() const { return _comm.nthread; }

    unsigned thread_num() const { return _comm.tid; }

    unsigned num_gangs() const { return _comm.ngang; }

    unsigned gang_num() const { return _comm.gid; }
    ...
    template <typename Arg> void broadcast_value(Arg& arg) const
    {
    ...
    {
        tci_comm_distribute_over_gangs(
            *this,
            n,
    ...
    {
        tci_comm_distribute_over_threads(
            *this,
            n,
            [](tci_comm*, uint64_t first, uint64_t last, void* payload)
            { (*(typename std::decay<Func>::type*)payload)(first, last); },
    ...
    template <typename Func>
    void distribute_over_gangs(const tci_range& m,
                               const tci_range& n,
                               Func&& func) const
    {
        tci_comm_distribute_over_gangs_2d(
            *this,
            m,
            n,
            [](tci_comm*,
               ...
               void* payload)
            {
                (*(typename std::decay<Func>::type*)payload)(mfirst,
                                                             mlast,
                                                             nfirst,
                                                             nlast);
            },
    ...
    template <typename Func>
    void distribute_over_threads(const tci_range& m,
                                 const tci_range& n,
                                 Func&& func) const
    {
        tci_comm_distribute_over_threads_2d(
            *this,
            m,
            n,
            [](tci_comm*,
    ...
    {
        typedef std::pair<typename std::decay<Func>::type, unsigned>
            payload_type;
    ...
    template <typename Func> void do_tasks(unsigned ntask, Func&& func) const
    {
