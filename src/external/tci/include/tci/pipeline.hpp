...
template <typename T, typename Alloc> class pipeline_base : private Alloc
{
  public:
...
template <typename T> class pipeline_base<T, void>
{
  public:
    ...
    ~pipeline_base() { tci_pipeline_destroy(_p); }
...
class pipeline : public pipeline_base<T, Alloc>
{
  public:
    ...
    {
      public:
        drain_guard(pipeline& p) : _p(p), _buffer(p.drain()) {}

        drain_guard(const drain_guard&) = delete;

        drain_guard(drain_guard&& other) : _p(other._p), _buffer(other._buffer)
        {
            other._p = nullptr;
        }
    ...
    {
      public:
        fill_guard(pipeline& p) : _p(p), _buffer(p.fill()) {}

        fill_guard(const fill_guard&) = delete;

        fill_guard(fill_guard&& other) : _p(other._p), _buffer(other._buffer)
        {
            other._p = nullptr;
        }
    ...
    template <typename Dummy = Alloc,
              typename = typename std::enable_if<
                  std::is_same<Alloc, void>::value>::type>
    ...
    const T* drain() { return (T*)tci_pipeline_drain(*this); }
    ...
    void drained(T* buffer) { tci_pipeline_drained(*this, buffer); }

    drain_guard guarded_drain() { return drain_guard(*this); }

    T* fill() { return (T*)tci_pipeline_fill(*this); }
    ...
    void filled(T* buffer) { tci_pipeline_filled(*this, buffer); }

    fill_guard guarded_fill() { return fill_guard(*this); }
};
