...
int tci_comm_init(tci_comm* comm,
                  tci_context* context,
                  unsigned nthread,
                  unsigned tid,
                  unsigned ngang,
...
int tci_comm_gang(tci_comm* parent,
                  tci_comm* child,
                  int type,
                  unsigned n,
                  unsigned bs)
{
    ...
    if (n == 1)
        return tci_comm_init(child, parent->context, nt, tid, 1, 0);
    if (n >= nt)
        return tci_comm_init(child, NULL, 1, 0, nt, tid);
    ...
    else
    {
        ...
        {
            tci_context_init(&contexts[block],
                             new_nthread,
...
void tci_distribute(unsigned n,
                    unsigned idx,
                    tci_comm* comm,
                    tci_range range,
                    tci_range_func func,
                    void* payload)
{
    ...
    func(comm,
         first * range.grain,
         TCI_MIN(last * range.grain, range.size),
         payload);
}

void tci_distribute_2d(unsigned num,
                       unsigned idx,
                       tci_comm* comm,
                       tci_range range_m,
                       tci_range range_n,
                       ...
                       void* payload)
{
    ...
    func(comm,
         mfirst * range_m.grain,
         TCI_MIN(mlast * range_m.grain, range_m.size),
         nfirst * range_n.grain,
         TCI_MIN(nlast * range_n.grain, range_n.size),
         payload);
}
...
static void tci_distribute_func(void* data_, size_t idx)
{
    ...
    uint64_t ngrain =
        (data->range->size + data->range->grain - 1) / data->range->grain;
    uint64_t first = (idx * ngrain) / data->n;
    uint64_t last = ((idx + 1) * ngrain) / data->n;

    data->func(data->comm,
               first * data->range->grain,
               TCI_MIN(last * data->range->grain, data->range->size),
               data->payload);
}
...
static void tci_distribute_2d_func(void* data_, size_t idx)
{
    ...
    uint64_t mgrain =
        (data->range_m->size + data->range_m->grain - 1) / data->range_m->grain;
    uint64_t ngrain =
        (data->range_n->size + data->range_n->grain - 1) / data->range_n->grain;
    ...
    data->func(data->comm,
               mfirst * data->range_m->grain,
               TCI_MIN(mlast * data->range_m->grain, data->range_m->size),
               nfirst * data->range_n->grain,
               TCI_MIN(nlast * data->range_n->grain, data->range_n->size),
               data->payload);
}
                       ...
                       void* payload)
{
    ...
    tci_distribute_2d_func_data data =
        {comm, func, m, n, &range_m, &range_n, payload};
    dispatch_queue_t queue =
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
...
void tci_distribute_2d(unsigned n,
                       unsigned idx,
                       tci_comm* comm,
                       tci_range range_m,
                       tci_range range_n,
...
void tci_comm_distribute_over_gangs(tci_comm* comm,
                                    tci_range range,
                                    tci_range_func func,
                                    void* payload)
{
...
void tci_comm_distribute_over_threads(tci_comm* comm,
                                      tci_range range,
                                      tci_range_func func,
                                      void* payload)
{
...
void tci_comm_distribute_over_threads_2d(tci_comm* comm,
                                         ...
                                         void* payload)
{
    tci_distribute_2d(comm->nthread,
                      comm->tid,
                      tci_single,
                      range_m,
                      range_n,
