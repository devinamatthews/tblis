...
inline void add(const communicator& comm,
                const scalar& alpha,
                const tensor& A_,
...
inline void
add(const communicator& comm, const scalar& alpha, const tensor& A, tensor&& B)
{
    add(comm, alpha, A, {0.0, A.type}, std::move(B));
}

inline void
add(const communicator& comm, const tensor& A, const scalar& beta, tensor&& B)
{
    add(comm, {1.0, A.type}, A, beta, std::move(B));
}

inline void add(const communicator& comm, const tensor& A, tensor&& B)
{
    add(comm, {1.0, A.type}, A, {0.0, A.type}, std::move(B));
}

inline void add(const scalar& alpha,
                const tensor& A,
                const label_vector& idx_A,
                const scalar& beta,
                tensor&& B,
...
inline void add(const tensor& A,
                const label_vector& idx_A,
                const scalar& beta,
                tensor&& B,
                const label_vector& idx_B)
...
inline void
add(const scalar& alpha, const tensor& A, const scalar& beta, tensor&& B)
{
    add(alpha, A, idx(A), beta, std::move(B), idx(B));
}

inline void add(const scalar& alpha, const tensor& A, tensor&& B)
{
    add(alpha, A, {0.0, A.type}, std::move(B));
}

inline void add(const tensor& A, const scalar& beta, tensor&& B)
{
    add({1.0, A.type}, A, beta, std::move(B));
}

inline void add(const tensor& A, tensor&& B)
{
