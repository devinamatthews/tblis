...
template <typename T = scalar> struct reduce_result
{
    T value;
    len_type idx;

    reduce_result(type_t) : value(), idx() {}

    operator const T&() const { return value; }
};

template <> struct reduce_result<scalar>
{
    scalar value;
    len_type idx;

    reduce_result(type_t type) : value(0.0, type), idx() {}
...
template <typename T = scalar>
reduce_result<T> reduce(const communicator& comm, reduce_t op, const tensor& A)
{
    reduce_result<T> result(A.type);
...
inline void reduce(reduce_t op,
                   const tensor& A,
                   const label_vector& idx_A,
                   scalar& result,
                   len_type& idx)
...
template <typename T = scalar>
reduce_result<T> reduce(reduce_t op, const tensor& A, const label_vector& idx_A)
{
    reduce_result<T> result(A.type);
...
inline void reduce(reduce_t op, const tensor& A, scalar& result, len_type& idx)
{
    reduce(op, A, tblis::idx(A), result, idx);
}

template <typename T>
void reduce(reduce_t op, const tensor& A, T& result, len_type& idx)
{
    scalar result_(0.0, A.type);
...
template <typename T>
reduce_result<T>
reduce(reduce_t op, dpd_marray_view<const T> A, const label_vector& idx_A)
{
...
template <typename T>
reduce_result<T>
reduce(reduce_t op, indexed_marray_view<const T> A, const label_vector& idx_A)
{
    reduce_result<T> result(type_tag<T>::value);
    reduce(op, A, idx_A, result.value, result.idx);
    return result;
...
template <typename Tensor> struct data_type_helper
{
    static void check(...);
    ...
    static std::decay_t<Eigen::TensorBase<D, A>::Scalar>
    check(Eigen::TensorBase<D, A>&);
...
template <typename T> struct data_type_helper2
{
    typedef T type;
};

template <> struct data_type_helper2<void>
{
};

template <typename T>
using data_type = typename data_type_helper2<
    typename data_type_helper<std::decay_t<T>>::type>::type;
...
#define TBLIS_ALIAS_REDUCTION(name, op, which)                              \
    ...
    inline auto name(const Tensor& t, Args&&... args)                       \
        -> decltype(reduce<internal::data_type<Tensor>>(                    \
            op,                                                             \
            t,                                                              \
            std::forward<Args>(args)...) which)                             \
    {                                                                       \
        return reduce<internal::data_type<Tensor>>(                         \
            op,                                                             \
            t,                                                              \
            std::forward<Args>(args)...) which;                             \
    }                                                                       \
                                                                            \
    template <typename Tensor, typename... Args>                            \
    inline auto name(const communicator& comm,                              \
                     const Tensor& t,                                       \
                     Args&&... args)                                        \
        -> decltype(reduce<internal::data_type<Tensor>>(                    \
            comm,                                                           \
