...
inline void scale(const communicator& comm,
                  const scalar& alpha,
                  tensor&& A,
...
inline void
scale(const communicator& comm, tensor&& A, const label_vector& idx_A)
{
    scale(comm, {1.0, A.type}, std::move(A), idx_A);
}

inline void scale(const communicator& comm, const scalar& alpha, tensor&& A)
{
    scale(comm, alpha, std::move(A), idx(A));
}

inline void scale(const communicator& comm, tensor&& A)
{
    scale(comm, {1.0, A.type}, std::move(A));
}

inline void scale(const scalar& alpha, tensor&& A, const label_vector& idx_A)
{
    scale(*(communicator*)nullptr, alpha, std::move(A), idx_A);
}

inline void scale(tensor&& A, const label_vector& idx_A)
{
    scale({1.0, A.type}, std::move(A), idx_A);
}

inline void scale(const scalar& alpha, tensor&& A)
{
    scale(alpha, std::move(A), idx(A));
}

inline void scale(tensor&& A)
{
...
{
    parallelize([&](const communicator& comm) { scale(comm, alpha, A, idx_A); },
