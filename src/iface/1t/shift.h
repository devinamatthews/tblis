...
inline void shift(const communicator& comm,
                  const scalar& alpha_,
                  const scalar& beta,
                  tensor&& A,
...
inline void shift(const communicator& comm, const scalar& alpha, tensor&& A)
{
    shift(comm, alpha, {1.0, A.type}, std::move(A));
}

inline void shift(const scalar& alpha,
                  const scalar& beta,
                  tensor&& A,
                  const label_vector& idx_A)
{
...
inline void shift(const scalar& alpha, tensor&& A, const label_vector& idx_A)
{
    shift(alpha, {1.0, A.type}, std::move(A), idx_A);
}

inline void shift(const scalar& alpha, const scalar& beta, tensor&& A)
{
    shift(alpha, beta, std::move(A), idx(A));
}

inline void shift(const scalar& alpha, tensor&& A)
{
...
template <typename T>
void
shift(T alpha, T beta, indexed_dpd_marray_view<T> A, const label_vector& idx_A)
{
    parallelize([&](const communicator& comm)
                { shift(comm, alpha, beta, A, idx_A); },
                tblis_get_num_threads());
