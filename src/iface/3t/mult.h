...
void tblis_tensor_mult(const tblis_comm* comm,
                       ...
                       tblis_tensor* C,
                       const label_type* idx_C);

#if defined(__cplusplus)

inline void mult(const communicator& comm,
                 const scalar& alpha,
                 const tensor& A,
...
inline void mult(const scalar& alpha,
                 const tensor& A,
                 const label_vector& idx_A,
                 const tensor& B,
                 const label_vector& idx_B,
...
inline void mult(const tensor& A,
                 const label_vector& idx_A,
                 const tensor& B,
                 const label_vector& idx_B,
                 const scalar& beta,
...
inline void
mult(const tensor& A, const tensor& B, const scalar& beta, const tensor& C)
{
    mult({1.0, A.type}, A, B, beta, C);
}

inline void
mult(const scalar& alpha, const tensor& A, const tensor& B, const tensor& C)
{
    mult(alpha, A, B, {0.0, A.type}, C);
}

inline void mult(const tensor& A, const tensor& B, const tensor& C)
{
