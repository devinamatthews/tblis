    ...
    A.for_each_block(
        [&](const marray_view<T>& local_A, const irrep_vector& irreps_A)
        {
            ...
            add(type_tag<T>::value,
                ...
                T(1),
                false,
                reinterpret_cast<char*>(local_A.data()),
                {},
                local_A.strides(),
                T(0),
                false,
...
void
dense_total_lengths_and_strides_helper(std::array<len_vector, N>& len,
                                       ...
                                       const Args&... args)
{
    ...
    {
        for (auto irrep : range(nirrep)) len[I][j] += A.length(j, irrep);
    }
...
template <typename... Args>
void
assign_irreps(int ndim, int irrep, int nirrep, stride_type block, Args&... args)
{
    int mask = nirrep - 1;
