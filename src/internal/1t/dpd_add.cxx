...
template <typename T>
void dpd_add_full(const communicator& comm, const config& cfg,
...
template <typename T>
void dpd_trace_block(const communicator& comm, const config& cfg,
                     T alpha, bool conj_A, const dpd_varray_view<const T>& A,
                     const dim_vector& idx_A_A,
                     const dim_vector& idx_A_AB,
                     T  beta, bool conj_B, const dpd_varray_view<      T>& B,
                     const dim_vector& idx_B_AB)
{
    ...
    for (stride_type block_AB = 0;block_AB < nblock_AB;block_AB++)
    {
        ...
        if (detail::is_block_empty(B, irreps_B)) continue;
        ...
        for (stride_type block_A = 0;block_A < nblock_A;block_A++)
        {
            ...
            add<T>(comm, cfg, len_A_only, {}, len_AB,
...
template <typename T>
void dpd_replicate_block(const communicator& comm, const config& cfg,
                         T alpha, bool conj_A, const dpd_varray_view<const T>& A,
                         const dim_vector& idx_A_AB,
                         T  beta, bool conj_B, const dpd_varray_view<      T>& B,
                         const dim_vector& idx_B_B,
...
template <typename T>
void dpd_transpose_block(const communicator& comm, const config& cfg,
                         T alpha, bool conj_A, const dpd_varray_view<const T>& A,
                         const dim_vector& idx_A_AB,
                         T  beta, bool conj_B, const dpd_varray_view<      T>& B,
                         const dim_vector& idx_B_AB)
