...
template <typename T>
void block_to_full(const communicator& comm,
                   const config& cfg,
                   ...
                   marray<T>& A2)
{
    ...
    for (auto i : range(A.num_indices()))
    {
        ...
        add(type_tag<T>::value,
            ...
            factor_A,
            false,
            reinterpret_cast<char*>(data_A),
            {},
            dense_stride_A,
            T(0),
            false,
            reinterpret_cast<char*>(data_A2),
            {},
            dense_stride_A2);
    }
}
...
template <typename T>
void full_to_block(const communicator& comm,
                   const config& cfg,
                   marray<T>& A2,
                   const indexed_marray_view<T>& A)
{
...
inline bool is_idx_dense(int)
{
    return true;
}

template <typename Array, typename... Args>
bool is_idx_dense(int i,
                  const Array& A,
...
template <int I, int N> void assign_dense_idx_helper(int, index_group<N>&) {}

template <int I, int N, typename T, typename... Args>
void assign_dense_idx_helper(int i,
                             index_group<N>& group,
                             const indexed_marray_view<T>& A,
                             const dim_vector& idx_A,
                             const Args&... args)
...
template <int N>
void assign_mixed_or_batch_idx_helper(int, int, int, index_group<N>&)
{
}

template <int N, typename T, typename... Args>
void assign_mixed_or_batch_idx_helper(int i,
                                      int pos,
                                      int j,
                                      index_group<N>& group,
                                      const indexed_marray_view<T>& A,
...
template <int N, typename T, typename... Args>
void assign_mixed_or_batch_idx(int i,
                               int pos,
                               index_group<N>& group,
                               const indexed_marray_view<T>& A,
                               const dim_vector& idx_A,
                               const Args&... args)
{
    assign_mixed_or_batch_idx_helper(i, pos, 0, group, A, idx_A, args...);
}
...
template <int N> struct index_group
{
    ...
    template <int... I> void fold(std::integer_sequence<int, I...>)
    {
        label_vector dense_idx;
        dense_idx.resize(dense_ndim);
    ...
    index_group(const indexed_marray_view<T>& A,
                const dim_vector& idx_A,
                const Args&... args)
    {
...
void get_mixed_lengths(len_vector& len,
                       dim_vector& off,
                       const Group& group,
                       int i,
                       const Args&... args)
{
    ...
    for (auto j : group.mixed_pos[i]) { len.push_back(group.batch_len[j]); }
...
template <int I, size_t N, typename Array, typename Group, typename... Args>
                              ...
                              const Args&... args)
{
    ...
    for (auto k : range(group.batch_idx[j].size()))
    {
        ...
        stride[I][group.batch_pos[j][k]] =
            group.batch_stride[group.batch_pos[j][k]];
    }
...
void set_mixed_indices_helper(std::array<len_vector, N>& idx,
                              std::array<stride_vector, N>& stride,
                              const viterator<0>& iter,
                              const dim_vector& off,
                              const Group& group,
                              int j,
                              const Args&... args)
{
    for (int k : range(group.mixed_pos[j].size()))
    {
        ...
        stride[I][group.mixed_pos[j][k]] =
            group.batch_stride[group.mixed_pos[j][k]];
    }
...
template <int N> struct index_set
{
...
template <int N> struct group_indices : std::vector<index_set<N>>
{
    template <typename Array, typename... Args>
    group_indices(type_t type, const Array& A, const Args&... args)
    {
        ...
        for (len_type i = 0; i < A.num_indices(); i++)
        {
            ...
            switch (type)
            {
                case TYPE_FLOAT:
                    idx.factor.reset(
                        reinterpret_cast<const float*>(&A.factor(0))[i]);
                    break;
                case TYPE_DOUBLE:
                    idx.factor.reset(
                        reinterpret_cast<const double*>(&A.factor(0))[i]);
                    break;
                case TYPE_SCOMPLEX:
                    idx.factor.reset(
                        reinterpret_cast<const scomplex*>(&A.factor(0))[i]);
                    break;
                case TYPE_DCOMPLEX:
                    idx.factor.reset(
                        reinterpret_cast<const dcomplex*>(&A.factor(0))[i]);
                    break;
            }

            while (iter.next())
            {
                set_mixed_indices(idx.idx,
                                  idx_stride,
                                  iter,
                                  mixed_off,
                                  args...);
                             ...
                             Args&&... args)
{
    ...
    TBLIS_ASSERT(group.mixed_pos[i].size() == group.mixed_stride[i].size());
...
void get_local_offset(const len_vector& idx,
                      const index_group<N>& group,
                      Args&&... args)
{
...
template <bool AIsRange, bool BIsRange, int NA, int NB, typename Body>
void for_each_match(stride_type& idx_A,
                    stride_type nidx_A,
                    const group_indices<NA>& indices_A,
                    int iA,
                    stride_type& idx_B,
                    stride_type nidx_B,
                    const group_indices<NB>& indices_B,
                    int iB,
                    Body&& body)
{
    while (idx_A < nidx_A && idx_B < nidx_B)
    {
        ...
        if (AIsRange)
        {
            while (next_A
                   < nidx_A
                   && indices_A[next_A].key[iA]
                   == indices_B[idx_B].key[iB])
                next_A++;
        }
        ...
        if (BIsRange)
        {
            while (next_B
                   < nidx_B
                   && indices_A[idx_A].key[iA]
                   == indices_B[next_B].key[iB])
                next_B++;
        }
...
void for_each_match(stride_type& idx_A,
                    ...
                    stride_type& idx_C,
                    stride_type nidx_C,
                    const group_indices<NC>& indices_C,
                    int iC,
                    Body&& body)
{
    while (idx_A < nidx_A && idx_B < nidx_B && idx_C < nidx_C)
    {
        ...
        if (CIsRange)
        {
            while (next_C
                   < nidx_C
                   && indices_A[idx_A].key[iA]
                   == indices_C[next_C].key[iC])
                next_C++;
        }

        call_match_body<AIsRange, BIsRange, CIsRange>{body,
                                                      next_A,
                                                      next_B,
                                                      next_C};
