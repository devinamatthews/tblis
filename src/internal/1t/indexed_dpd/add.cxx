...
template <typename T>
void add_full(const communicator& comm,
              const config& cfg,
              T alpha,
              bool conj_A,
                 ...
                 const dim_vector& idx_B_AB)
{
    ...
    if (group_A.dense_ndim == 0 && irrep_A != 0)
        return;
    ...
    comm.do_tasks_deferred(
        ...
        [&](communicator::deferred_task_set& tasks)
        {
            for_each_match<true, false>(
                ...
                [&](stride_type next_A)
                {
                    if (indices_B[idx_B].factor.is_zero())
                        return;

                    for (stride_type block_AB = 0;
                         block_AB < group_AB.dense_nblock;
                         block_AB++)
                    {
                        tasks.visit(
                            task++,
                            [&, idx_A, idx_B, block_AB, next_A](
                                const communicator& subcomm)
                            {
                                auto local_irreps_A = irreps_A;
                                auto local_irreps_B = irreps_B;

                                assign_irreps(group_AB.dense_ndim,
                                              irrep_AB,
                                              nirrep,
                                              block_AB,
                                              local_irreps_A,
                                ...
                                if (is_block_empty(dpd_B, local_irreps_B))
                                    return;

                                marray_view<char> local_B =
                                    dpd_B(local_irreps_B);

                                for (stride_type block_A = 0;
                                     block_A < group_A.dense_nblock;
                                     block_A++)
                                {
                                    assign_irreps(group_A.dense_ndim,
                                                  irrep_A,
                                                  nirrep,
                                                  block_A,
                                                  local_irreps_A,
                                    ...
                                    if (is_block_empty(dpd_A, local_irreps_A))
                                        continue;

                                    marray_view<const char> local_A =
                                        dpd_A(local_irreps_A);

                                    len_vector len_AB;
                                    stride_vector stride_A_AB, stride_B_AB;
                                    stride_type off_A_AB, off_B_AB;
                                    get_local_geometry(indices_A[idx_A].idx[0],
                                                       group_AB,
                                                       len_AB,
                                                       local_A,
                                                       stride_A_AB,
                                                       ...
                                                       stride_B_AB,
                                                       1);
                                    get_local_offset(indices_A[idx_A].idx[0],
                                                     group_AB,
                                                     local_A,
                                                     off_A_AB,
                                                     0,
                                    ...
                                    get_local_geometry(indices_A[idx_A].idx[1],
                                                       group_A,
                                                       len_A,
                                                       local_A,
                                                       stride_A_A,
                                    ...
                                    auto data_B = dpd_B.data()
                                                + (local_B.data()
                                                   - dpd_B.data()
                                                   + indices_B[idx_B].offset
                                                   + off_B_AB)
                                                * ts;

                                    for (auto local_idx_A = idx_A;
                                         local_idx_A < next_A;
                                         local_idx_A++)
                                    {
                                        auto factor =
                                            alpha
                                            * indices_A[local_idx_A].factor
...
void replicate_block(type_t type,
                     const communicator& comm,
                     const config& cfg,
                     const scalar& alpha,
                     bool conj_A,
                     ...
                     const dim_vector& idx_B_AB)
{
    ...
    if (group_B.dense_ndim == 0 && irrep_B != 0)
        return;
    if (group_AB.dense_ndim == 0 && irrep_AB != 0)
        return;
    ...
    comm.do_tasks_deferred(
        nidx_B * group_AB.dense_nblock * group_B.dense_nblock,
        group_AB.dense_size * group_B.dense_size * inout_ratio,
        [&](communicator::deferred_task_set& tasks)
        {
            for_each_match<false, true>(
                idx_A,
                nidx_A,
                indices_A,
                0,
                ...
                [&](stride_type next_B)
                {
                    if (indices_A[idx_A].factor.is_zero())
                        return;

                    for (auto local_idx_B = idx_B; local_idx_B < next_B;
                         local_idx_B++)
                    {
                        auto factor = alpha
                                    * indices_A[idx_A].factor
                        ...
                        for (stride_type block_AB = 0;
                             ...
                             block_AB++)
                        {
                            for (stride_type block_B = 0;
                                 block_B < group_B.dense_nblock;
                                 block_B++)
                            {
                                tasks.visit(
                                    task++,
                                    [&,
                                     ...
                                     local_idx_B,
                                     block_AB,
                                     block_B](const communicator& subcomm)
                                    {
                                        auto local_irreps_A = irreps_A;
                                        auto local_irreps_B = irreps_B;
                                        ...
                                        assign_irreps(group_B.dense_ndim,
                                                      irrep_B,
                                                      nirrep,
                                                      block_B,
                                                      local_irreps_B,
                                        ...
                                        if (is_block_empty(dpd_A,
                                                           local_irreps_A))
                                            return;
                                        if (is_block_empty(dpd_B,
                                                           local_irreps_B))
                                            return;
                                        ...
                                        auto data_A = dpd_A.data()
                                                    + (local_A.data()
                                                       - dpd_A.data()
                                                       + indices_A[idx_A].offset
                                                       + off_A_AB)
                                                    * ts;
                                        auto data_B =
                                            dpd_B.data()
...
void transpose_block(type_t type,
                     const communicator& comm,
                     const config& cfg,
                     const scalar& alpha,
                     bool conj_A,
