...
template <typename T>
void dot_full(const communicator& comm,
              const config& cfg,
              bool conj_A,
               ...
               char* result)
{
    ...
    comm.do_tasks_deferred(
        std::min(nidx_A, nidx_B) * group_AB.dense_nblock,
        ...
        [&](communicator::deferred_task_set& tasks)
        {
            for_each_match<false, false>(
                ...
                {
                    auto factor =
                        indices_A[idx_A].factor * indices_B[idx_B].factor;
                    if (factor.is_zero())
                        return;
    ...
    if (comm.master())
        local_result.store(type, result);
}
         ...
         char* result)
{
    ...
    {
        if (comm.master())
            memset(result, 0, type_size[type]);
        comm.barrier();
        return;
    }
