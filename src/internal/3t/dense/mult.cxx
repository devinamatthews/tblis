               ...
               const stride_vector& stride_C_BC_)
{
    ...
    unsigned nt_l, nt_m;
    std::tie(nt_l, nt_m) =
        partition_2x2(comm.num_threads(), m2 * n2, m2 * n2, m * n, m * n);
    ...
    subcomm.distribute_over_gangs(
        ...
        [&](len_type l_min, len_type l_max)
        {
            viterator<3> iter_ABC(
                stl_ext::appended(stl_ext::permuted(len_AC, reorder_AC),
               ...
               const stride_vector& stride_C_BC)
{
    ...
    if (pack_M_3d)
        std::rotate(reorder_AC.begin() + 1,
                    reorder_AC.begin() + unit_A_AC,
                    reorder_AC.end());

    if (pack_N_3d)
        std::rotate(reorder_BC.begin() + 1,
                    reorder_BC.begin() + unit_B_BC,
                    reorder_BC.end());

    if (pack_K_3d)
        std::rotate(reorder_AB.begin() + 1,
                    reorder_AB.begin() + std::max(unit_A_AB, unit_B_AB),
                    reorder_AB.end());

    tensor_matrix at(alpha,
                     conj_A,
               ...
               const stride_vector& stride_C_ABC_)
{
    ...
    auto reorder_AB = detail::sort_by_stride(stride_A_AB, stride_B_AB);
    auto reorder_ABC =
        detail::sort_by_stride(stride_C_ABC, stride_A_ABC, stride_B_ABC);
    ...
    unsigned nt_l, nt_m;
    std::tie(nt_l, nt_m) =
        partition_2x2(comm.num_threads(), l * m2, l * m2, m, m);
    ...
    subcomm.distribute_over_gangs(
        ...
        [&](len_type l_min, len_type l_max)
        {
            ...
            viterator<3> iter_ABC(
                stl_ext::appended(stl_ext::permuted(len_ABC, reorder_ABC),
                                  stl_ext::permuted(len_AC, reorder_AC)),
               ...
               const stride_vector& stride_C_ABC_)
{
    ...
    unsigned nt_l, nt_m;
    std::tie(nt_l, nt_m) = partition_2x2(comm.num_threads(),
                                         l * m2 * n2,
                                         l * m2 * n2,
                                         m * n,
                                         m * n);
...
template <typename T>
void mult_blas(const communicator& comm,
               const config& cfg,
               ...
               const stride_vector& stride_C_ABC_)
{
    ...
    len_ABC.push_back(1);
    auto stride_A_AB = stride_A_AB_;
    stride_A_AB.push_back(1);
    auto stride_B_AB = stride_B_AB_;
    stride_B_AB.push_back(1);
    auto stride_A_AC = stride_A_AC_;
    stride_A_AC.push_back(1);
    auto stride_C_AC = stride_C_AC_;
    stride_C_AC.push_back(1);
    auto stride_B_BC = stride_B_BC_;
    stride_B_BC.push_back(1);
    auto stride_C_BC = stride_C_BC_;
    stride_C_BC.push_back(1);
    auto stride_A_ABC = stride_A_ABC_;
    stride_A_ABC.push_back(1);
    auto stride_B_ABC = stride_B_ABC_;
    stride_B_ABC.push_back(1);
    auto stride_C_ABC = stride_C_ABC_;
    stride_C_ABC.push_back(1);
    ...
    comm.broadcast(
        [&](marray<T>& ar, marray<T>& br, marray<T>& cr)
        {
            ...
            while (it.next(A1, B1, C1))
            {
                ...
                normal_matrix at(T(1),
                                 false,
                                 am.length(0),
                                 am.length(1),
                                 reinterpret_cast<char*>(am.data()),
                                 am.stride(0),
                                 am.stride(1));

                normal_matrix bt(T(1),
                                 false,
                                 bm.length(0),
                                 bm.length(1),
                                 reinterpret_cast<char*>(bm.data()),
                                 bm.stride(0),
                                 bm.stride(1));

                normal_matrix ct(T(0),
                                 false,
                                 cm.length(0),
                                 cm.length(1),
                                 reinterpret_cast<char*>(cm.data()),
                                 cm.stride(0),
                                 cm.stride(1));
              ...
              const stride_vector& stride_C_ABC)
{
    ...
    for (auto i : range(1, stride_A_ABC.size()))
        stride_A1.push_back(stride_A_ABC[i] * ts);
    for (auto i : range(1, stride_B_ABC.size()))
        stride_B1.push_back(stride_B_ABC[i] * ts);
    for (auto i : range(1, stride_C_ABC.size()))
        stride_C1.push_back(stride_C_ABC[i] * ts);
          ...
          const stride_vector& stride_C_ABC)
{
    ...
    scalar zero(0, type);
    scalar sum(0, type);
    auto stride_A_ABC_ts = stride_A_ABC;
    for (auto& s : stride_A_ABC_ts) s *= ts;
    auto stride_B_ABC_ts = stride_B_ABC;
    for (auto& s : stride_B_ABC_ts) s *= ts;
    auto stride_C_ABC_ts = stride_C_ABC;
    for (auto& s : stride_C_ABC_ts) s *= ts;
    viterator<3> iter_ABC(len_ABC,
                          stride_A_ABC_ts,
                          stride_B_ABC_ts,
                          stride_C_ABC_ts);
