...
using addf_sum_ukr_t = void (*)(len_type m,
                                len_type n,
                                const void* alpha,
                                bool conj_A,
                                const void** A,
...
void addf_sum_ukr_def(len_type m,
                      len_type n,
                      const void* alpha_,
                      bool conj_A,
                      const void** A_,
                      ...
                      stride_type inc_C)
{
    ...
    if (beta == T(0))
    {
        for (len_type i = 0; i < m; i++) C[i * inc_C] = T(0);
    }
    else
    {
...
using addf_rep_ukr_t = void (*)(len_type m,
                                len_type n,
                                const void* alpha,
                                bool conj_A,
                                const void* A,
...
template <typename Config, typename T>
void addf_rep_ukr_def(len_type m,
                      len_type n,
                      const void* alpha_,
                      bool conj_A,
                      const void* A_,
                      ...
                      stride_type inc_C)
{
    ...
    else
    {
        for (len_type j = 0; j < n; j++)
            for (len_type i = 0; i < m; i++)
                C[j][i * inc_C] = alpha_B[j]
                                * conj(conj_A, A[i * inc_A])
                                + beta
                                * conj(conj_C, C[j][i * inc_C]);
    }
}
