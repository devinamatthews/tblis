...
using trans_ukr_t = void (*)(len_type m,
                             len_type n,
                             const void* alpha,
                             bool conj_A,
                             const void* A,
                ...
                else
                {
                    for (len_type i = 0; i < MR; i++)
                        for (len_type j = 0; j < NR; j++)
                            B[j * cs_B + i * rs_B] =
                                alpha
                                * conj(A[i * rs_A + j * cs_A])
                                + beta
                                * conj(B[j * cs_B + i * rs_B]);
                }
            }
            else if (is_complex_v<T> && conj_B)
            {
                if (rs_B == 1 && rs_A == 1)
                {
                    for (len_type i = 0; i < MR; i++)
        ...
        if (beta == T(0))
        {
            for (len_type i = 0; i < m; i++)
                for (len_type j = 0; j < n; j++)
                    B[j * cs_B + i * rs_B] =
                        alpha * conj(conj_A, A[i * rs_A + j * cs_A]);
        }
        else
        {
            for (len_type i = 0; i < m; i++)
                for (len_type j = 0; j < n; j++)
                    B[j * cs_B + i * rs_B] =
                        alpha
                        * conj(conj_A, A[i * rs_A + j * cs_A])
                        + beta
                        * conj(conj_B, B[j * cs_B + i * rs_B]);
        }
    }
}
