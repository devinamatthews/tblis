...
using add_ukr_t = void (*)(len_type n,
                           const void* alpha,
                           bool conj_A,
                           const void* A,
                           stride_type inc_A,
                 ...
                 stride_type inc_B)
{
    ...
    if (beta == T(0))
    {
        if (is_complex_v<T> && conj_A)
        {
            ...
            {
#pragma omp simd
                for (len_type i = 0; i < n; i++) B[i] = alpha * conj(A[i]);
            }
            else
            {
...
#pragma omp simd
            ...
            else
            {
                for (len_type i = 0; i < n; i++)
                    B[i * inc_B] =
                        alpha * conj(A[i * inc_A]) + beta * conj(B[i * inc_B]);
            }
        }
...
#pragma omp simd
            ...
            else
            {
                for (len_type i = 0; i < n; i++)
                    B[i * inc_B] =
                        alpha * A[i * inc_A] + beta * conj(B[i * inc_B]);
            }
        }
...
#pragma omp simd
            ...
            else
            {
                for (len_type i = 0; i < n; i++)
                    B[i * inc_B] =
                        alpha * conj(A[i * inc_A]) + beta * B[i * inc_B];
            }
        }
        else
        {
