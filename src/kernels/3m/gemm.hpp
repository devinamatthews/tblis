...
#define EXTERN_UPDATE_NN_UKR(name)             \
    extern void name(tblis::stride_type m,     \
                     ...
                     const void* beta,         \
                     void* c,                  \
                     tblis::stride_type rs_c,  \
                     tblis::stride_type cs_c);

using update_nn_ukr_t = void (*)(stride_type m,
                                 stride_type n,
                                 const void* ab,
                                 const void* d,
                                 stride_type inc_d,
...
#define EXTERN_UPDATE_SS_UKR(name)                       \
    extern void name(tblis::stride_type m,               \
                     ...
                     const void* beta,                   \
                     void* c,                            \
                     const tblis::stride_type* rscat_c,  \
                     const tblis::stride_type* cscat_c);

using update_ss_ukr_t = void (*)(stride_type m,
                                 stride_type n,
                                 const void* ab,
                                 const void* beta,
                                 void* c,
                                 const stride_type* rscat_c,
                                 const stride_type* cscat_c);
...
using gemm_ukr_t = void (*)(stride_type m,
                            stride_type n,
                            stride_type k,
                            const void* a,
                            const void* b,
...
void update_nn_ukr_def(stride_type m,
                       stride_type n,
                       const void* p_ab_,
                       const void* p_d_,
                       stride_type inc_d,
...
template <typename Config, typename T>
void update_ss_ukr_def(stride_type m,
                       stride_type n,
                       const void* p_ab_,
                       const void* beta_,
                       void* p_c_,
                       const stride_type* rscat_c_,
...
template <typename Config, typename T>
void gemm_ukr_def(stride_type m,
                  stride_type n,
                  stride_type k,
                  const void* p_a_,
                  const void* p_b_,
                  ...
                  auxinfo_t*)
{
    ...
    while (k-- > 0)
    {
        for (int i = 0; i < MR; i++)
#pragma omp simd
            for (int j = 0; j < NR; j++) p_ab[i * NR + j] += p_a[i] * p_b[j];
