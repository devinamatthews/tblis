...
class abstract_matrix
{
  ...
  protected:
    ...
    void (*gemm_)(abstract_matrix&,
                  const communicator& comm,
                  const config& cfg,
                  MemoryPool& pool,
                  const abstract_matrix& A,
                  const abstract_matrix& B) = nullptr;

    abstract_matrix(const scalar& scalar,
                    bool conj,
                    bool row_major,
                    len_type m,
                    len_type n,
    ...
    char*
    get_buffer(const communicator& comm, stride_type nelem, MemoryPool& pool)
    {
        if (pack_size_ < nelem)
        {
            if (comm.master())
    ...
    char* get_buffer() const { return pack_ptr_; }

    stride_type get_buffer_size() const { return pack_size_; }
  ...
  public:
    abstract_matrix(type_t type) : scale_(1, type) {}

    type_t type() const { return scale_.type; }
    ...
    void shift(int dim, len_type n) { shift_and_resize(dim, n, length(dim)); }
    ...
    void transpose() { transposed_ = !transposed_; }
    ...
    abstract_matrix
    pack(const communicator& comm, const config& cfg, int mat, MemoryPool& pool)
    {
        TBLIS_ASSERT(pack_, "This matrix cannot be packed.");
        return pack_(*this, comm, cfg, mat, pool);
    }

    void gemm(const communicator& comm,
              const config& cfg,
              MemoryPool& pool,
              const abstract_matrix& A,
...
class abstract_matrix_adapter : public abstract_matrix
{
  protected:
    ...
    template <typename... Args>
    abstract_matrix_adapter(const tblis_scalar& alpha,
                            bool conj,
                            bool row_major,
                            len_type m,
                            len_type n,
                            Args&&... args)
    : abstract_matrix(alpha,
                      conj,
                      row_major,
                      m,
                      n,
                      std::make_shared<matrix_implementation_wrapper>(
...
template <typename Derived>
class abstract_matrix_adapter<Derived, void> : public abstract_matrix
{
  protected:
    abstract_matrix_adapter(const tblis_scalar& alpha,
                            bool conj,
