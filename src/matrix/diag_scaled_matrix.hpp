...
struct diag_scaled_matrix_impl
{
    ...
    : data_(ptr),
      stride_{rs, cs},
      diag_{rdiag, cdiag},
      diag_stride_{rds, cds}
    {
    }
};

class diag_scaled_matrix
: public abstract_matrix_adapter<diag_scaled_matrix, diag_scaled_matrix_impl>
{
  protected:
        ...
        comm.distribute_over_threads(
                ...
                len_type k_last)
            {
                for (len_type m_off = m_first; m_off < m_last; m_off++)
                {
                    ...
                    const char* p_d = A.diag(trans);
                    if (p_d)
                        p_d += m_off * inc_d;
                    const char* p_e = A.diag(!trans);
                    if (p_e)
                        p_e += k_first * inc_e;
    ...
    static void do_gemm(diag_scaled_matrix& C,
                        const communicator& comm,
                        const config& cfg,
                        MemoryPool&,
                        const packed_matrix& A,
                        const packed_matrix& B)
    {
            ...
            [&](len_type n_first, len_type n_last)
            {
                ...
                for (len_type n_off = n_first; n_off < n_last; n_off++)
                    for (len_type m_off = m_first; m_off < m_last; m_off++)
                    {
                        ...
                        const char* p_d = C.diag(0);
                        if (p_d)
                            p_d += m_off * inc_d;
                        const char* p_e = C.diag(1);
                        if (p_e)
                            p_e += n_off * inc_e;
                        char* p_c = C.data() + m_off * rs_c + n_off * cs_c;
  ...
  public:
    diag_scaled_matrix(const tblis_scalar& alpha,
                       bool conj,
                       len_type m,
                       len_type n,
                       char* ptr,
    ...
    diag_scaled_matrix(type_t type,
                       bool conj,
                       len_type m,
                       len_type n,
                       char* ptr,
    ...
    {
        return impl().data_
             + (stride(0) * offset(0) + stride(1) * offset(1))
             * type_size[type()];
    }
    ...
    char* diag(int dim) const
    {
        ...
        return diag ? diag + diag_stride(dim) * offset(dim) * type_size[type()]
                    : nullptr;
    }
};
