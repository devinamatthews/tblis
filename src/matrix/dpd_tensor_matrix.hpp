...
struct dpd_tensor_matrix_impl
{
    ...
    dpd_tensor_matrix_impl(const dpd_marray_view<char>& other,
                           ...
                           bool pack_n_3d)
    : tensor_(other),
      dims_{row_inds, col_inds},
      extra_dims_(extra_inds),
      extra_irreps_(extra_irreps),
      extra_idx_(extra_idx),
      irrep_{col_irrep ^ other.irrep(), col_irrep},
      pack_3d_{pack_m_3d, pack_n_3d}
    {
        for (auto irrep : extra_irreps_) irrep_[0] ^= irrep;

        TBLIS_ASSERT(
            (int)(dims_[0].size() + dims_[1].size() + extra_dims_.size())
            == other.dimension());
        ...
        for (auto dim : {0, 1})
        {
            ...
            {
                MArray::irrep_iterator it(irrep_[dim],
                                          nirrep,
                                          dims_[dim].size());
...
template <typename T> struct is_dpd_tensor_helper : std::false_type
{
};
...
: public abstract_matrix_adapter<dpd_tensor_matrix, dpd_tensor_matrix_impl>
{
  protected:
                                   ...
                                   MemoryPool& pool)
    {
        ...
        const len_type MR =
            (!trans ? cfg.gemm_mr.def(type) : cfg.gemm_nr.def(type));
        const len_type ME =
            (!trans ? cfg.gemm_mr.extent(type) : cfg.gemm_nr.extent(type));
        ...
        const len_type m_p =
            A.get_patches(trans, first_patch[trans], off_patch[trans], MR, ME);
        const len_type k_p = A.get_patches(!trans,
                                           first_patch[!trans],
                                           off_patch[!trans],
                                           KE,
                                           KE);
        ...
        const len_type MC =
            std::max(!trans ? cfg.gemm_mc.max(type) : cfg.gemm_nc.max(type),
                     m_p);
        const len_type KC = std::max(cfg.gemm_kc.max(type), k_p);
        const stride_type nelem = MC
                                * KC
                                + std::max(MC, KC)
                                * TBLIS_MAX_UNROLL
                                + 2
                                * size_as_type<stride_type>(MC + KC, type);

        packed_matrix P(type,
                        !trans ? m : k_p,
                        !trans ? k_p : m,
                        A.get_buffer(comm, nelem, pool),
                        ps_ap);

        stride_type* rscat =
            convert_and_align<stride_type>(P.data() + m_p * k_p * ts);
        ...
        A.for_each_patch(
            ...
            [&](int m_patch, len_type m_patch_size, len_type m_off_patch)
            {
                ...
                A.for_each_patch(
                    ...
                    off_patch[!trans],
                    [&](int k_patch,
                        len_type k_patch_size,
                        len_type k_off_patch)
                    {
                        char* p_a0 = !trans ? A.fill_block_scatter(comm,
                                                                   MR,
                                                                   KR,
                                                                   m_patch,
                                                                   m_off_patch,
    ...
    static void do_gemm(abstract_matrix& C_,
                        const communicator& comm,
                        const config& cfg,
                        MemoryPool& pool,
                        const abstract_matrix& A_,
                        const abstract_matrix& B_)
    {
        ...
        stride_type* rscat =
            convert_and_align<stride_type>(C.get_buffer(comm, nelem, pool));
        ...
        C.for_each_patch(
            ...
            [&](int n_patch, len_type n_patch_size, len_type n_off_patch)
            {
                ...
                C.for_each_patch(0,
                                 ...
                                 off_patch[0],
                                 [&](int m_patch,
                                     len_type m_patch_size,
                                     len_type m_off_patch)
                                 {
                                     auto p_c0 =
                                         C.fill_block_scatter(comm,
                                                              MR,
                                                              NR,
                                                              m_patch,
                                                              m_off_patch,
                                     ...
                                     p_a0 +=
                                         ceil_div(m_patch_size, MR) * ps_a * ts;
                                 });
  ...
  public:
    dpd_tensor_matrix(const tblis_scalar& alpha,
                      bool conj,
    ...
    char* fill_block_scatter(const communicator& comm,
                             ...
                             len_type m_patch_size,
                             len_type n_patch,
                             len_type n_off_patch,
                             len_type n_patch_size,
                             stride_type* rscat,
                             stride_type* cscat,
                             stride_type* rbs,
                             stride_type* cbs)
    {
        ...
        for (auto dim : {0, 1})
        {
            ...
            TBLIS_ASSERT(
                idx < (1u << irrep_bits * (std::max<int>(dims.size(), 1) - 1)));
            irreps[dims[0]] = irrep(dim);
            for (auto i : range(1, dims.size()))
            {
                irreps[dims[0]] ^= irreps[dims[i]] = idx & irrep_mask;
                idx >>= irrep_bits;
            }
        }
        ...
        if (comm.master())
        {
            tensor_matrix::fill_block_scatter(type,
                                              len_m,
                                              stride_m,
                                              MR,
                                              m_off_patch,
    ...
    const dim_vector& extra_dims() const { return impl().extra_dims_; }

    const irrep_vector& extra_irreps() const { return impl().extra_irreps_; }

    const len_vector& extra_idx() const { return impl().extra_idx_; }

    const dpd_marray_view<char>& tensor() const { return impl().tensor_; }
