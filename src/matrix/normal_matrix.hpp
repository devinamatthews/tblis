...
class normal_matrix
: public abstract_matrix_adapter<normal_matrix, normal_matrix_impl>
{
  protected:
  ...
  public:
    normal_matrix(const scalar& alpha,
                  bool conj,
                  len_type m,
                  len_type n,
                  char* ptr,
                  stride_type rs,
                  stride_type cs)
    : abstract_matrix_adapter(alpha, conj, cs == 1 && n > 1, m, n, ptr, rs, cs)
    {
        pack_ = &do_pack;
        gemm_ = &do_gemm;
    }
            ...
            [&](len_type n_first, len_type n_last)
            {
                for (len_type n_off = n_first; n_off < n_last; n_off++)
                    for (len_type m_off = m_first; m_off < m_last; m_off++)
                    {
                        ...
                        char* p_c1 =
                            p_c + m_off * MR * rs * ts + n_off * NR * cs * ts;
