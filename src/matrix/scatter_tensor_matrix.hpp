...
struct scatter_tensor_matrix_impl
{
    ...
    scatter_tensor_matrix_impl(char* ptr,
                               ...
                               bool pack_n_3d)
    : data_(ptr),
      outer_len_{rscat.length(), cscat.length()},
      scatter_{rscat.data(), cscat.data()},
...
: public abstract_matrix_adapter<scatter_tensor_matrix,
                                 scatter_tensor_matrix_impl>
{
  protected:
  ...
  public:
    scatter_tensor_matrix(const scalar& alpha,
                          bool conj,
                          len_array len_m,
                          len_array len_n,
                          char* ptr,
                              ...
                              pack_n_3d)
    {
        ...
        bool row_major =
            !strides(1).empty() && strides(1)[0] == 1 && inner_length(1) > 1;
        ...
        if (comm.master())
        {
            for (auto dim : {0, 1})
            {
                ...
                while (len > 0)
                {
                    ...
                    tensor_matrix::fill_scatter(type(),
                                                lens,
                                                strides,
                                                BS,
                                                off,
                ...
                tensor_matrix::fill_block_stride(BS,
                                                 length(dim),
                                                 dim == 0 ? rscat : cscat,
                                                 dim == 0 ? rbs : cbs);
            }
