...
struct tensor_matrix_impl
{
    ...
    : data_(ptr),
      lens_{len_m, len_n},
      strides_{stride_m, stride_n},
      pack_3d_{pack_m_3d, pack_n_3d}
    {
    }
};

template <typename T> struct is_tensor_helper : std::false_type
{
};
...
class tensor_matrix
: public abstract_matrix_adapter<tensor_matrix, tensor_matrix_impl>
{
  protected:
  ...
  public:
    tensor_matrix(const scalar& alpha,
                  bool conj,
                  const len_vector& len_m,
                  const len_vector& len_n,
                  char* ptr,
                              ...
                              pack_n_3d)
    {
        ...
        bool row_major = !strides(1).empty() && strides(1)[0] == 1 && n > 1;
    ...
    static void fill_scatter_3d(type_t type,
                                len_type m0,
                                len_vector len,
                                len_type s0,
            ...
            for (len_type b = b_min; b < b_max && it.next(pos); b++)
            {
                ...
                for (len_type i1 = min1; i1 < max1; i1 += BS1)
                    for (len_type i0 = 0; i0 < r0; i0 += BS0)
                        for (len_type j1 = 0; j1 < std::min(BS1, max1 - i1);
                             j1++)
                            for (len_type j0 = 0; j0 < BS0; j0++)
                                scat[idx++] =
                                    pos + (i0 + j0) * s0 + (i1 + j1) * s1;
            }
    ...
    static void fill_scatter(type_t type,
                             len_vector len,
                             stride_vector stride,
                             len_type BS,
                             stride_type off,
    ...
    static void fill_block_scatter(type_t type,
                                   len_vector len,
                                   stride_vector stride,
                                   len_type BS,
                                   stride_type off,
    ...
    static void pack(const communicator& comm,
                     const config& cfg,
                     const scalar& alpha,
                     bool conj,
                     bool trans,
                     ...
                     stride_type ps_ap)
    {
        ...
        comm.distribute_over_threads(
            ...
            {
                char* p_ap1 =
                    p_ap + (m_first / MR) * ps_ap * ts + k_first * ME * ts;
    ...
    static void gemm(const communicator& comm,
                     const config& cfg,
                     const scalar& beta,
                     bool conj,
                     len_type m,
    ...
    char* data() const { return impl().data_; }
