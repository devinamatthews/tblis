...
template <typename U, typename T> U* convert_and_align(T* x)
{
    uintptr_t off = (reinterpret_cast<uintptr_t>(x)) % alignof(U);
    return reinterpret_cast<U*>(reinterpret_cast<char*>(x)
                                + (off == 0 ? 0 : alignof(U) - off));
}

template <typename T, typename U> constexpr size_t size_as_type(size_t n)
{
    return ceil_div(n * sizeof(T) + alignof(T), sizeof(U));
}

template <typename T> constexpr size_t size_as_type(size_t n, type_t type)
{
