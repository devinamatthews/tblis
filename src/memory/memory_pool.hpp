  ...
  public:
    class Block
    {
      ...
      public:
        ...
        template <typename T = void> T* get() { return static_cast<T*>(_ptr); }

        template <typename T = void> const T* get() const
        {
  ...
  protected:
    void* acquire(size_t& size, size_t alignment)
    {
        ...
        if (!_free_list.empty())
        {
            ...
            if (entry.second
                >= size
                && (reinterpret_cast<uintptr_t>(entry.first) & (alignment - 1))
                == 0)
            {
