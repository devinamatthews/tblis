...
template <typename T> class stack_allocator
{
  protected:
    class stack
    {
      public:
        ...
        stack(size_t size) : _mem(::operator new(size)), _size(size), _pos(0) {}

        ~stack()
        {
            if (_pos > 0)
                throw std::runtime_error(
                    "attempting to delete a stack which is not empty");
        ...
        template <typename U> bool deallocate(U* t_ptr, size_t size)
        {
            size_t size_bytes = size * sizeof(U);
            char* ptr = (char*)t_ptr;
            ...
            if (_pos != size_t(ptr - _mem) + size_bytes)
                throw std::runtime_error(
                    "attempting to free memory which is not at the top of the stack");

            if (ptr_size(ptr) != size)
                throw std::runtime_error(
                    "attempting to free memory of wrong size");
      ...
      protected:
        ...
        size_t& ptr_size(char* ptr) { return *(size_t*)(ptr - sizeof(size_t)); }
  ...
  public:
    ...
    : _stack_size(stack_size),
      ...
      _align(alignment),
      _stacks(std::make_shared<std::vector<stack>>()),
      _stack(0)
    {
    ...
    : _stack_size(other._stack_size),
      ...
      _align(std::max(alignof(T), other._align)),
    ...
    template <typename U>
    friend bool operator==(const stack_allocator& a,
                           const stack_allocator<U>& b)
    {
