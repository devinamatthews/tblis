...
template <template <typename> class NodeType, typename T> struct node_depth
{
    static constexpr int value =
        node_depth<NodeType, decltype(std::declval<T>().child)>::value + 1;
};

template <typename T> struct node_depth<partition_gemm_mc, partition_gemm_mc<T>>
{
    static constexpr int value = 0;
};

template <typename T> struct node_depth<partition_gemm_nc, partition_gemm_nc<T>>
{
    static constexpr int value = 0;
};

template <typename T> struct node_depth<partition_gemm_kc, partition_gemm_kc<T>>
{
    static constexpr int value = 0;
};

template <typename T> struct node_depth<partition_gemm_mr, partition_gemm_mr<T>>
{
    static constexpr int value = 0;
};

template <typename T> struct node_depth<partition_gemm_nr, partition_gemm_nr<T>>
{
    static constexpr int value = 0;
};
...
template <int Depth, typename T> struct node_type
{
    typedef
        typename node_type<Depth - 1, decltype(std::declval<T>().child)>::type
            type;
};

template <typename T> struct node_type<0, T>
{
    typedef T type;
};

template <int Depth> struct node_at_depth
{
    template <typename T>
...
template <> struct node_at_depth<0>
{
    template <typename T> T& operator()(T& tree) const { return tree; }
};
...
template <typename Child> struct gemm
{
    Child child;
...
using GotoGEMM = gemm<partition_gemm_nc<partition_gemm_kc<
    pack_b<BuffersForB,
           partition_gemm_mc<pack_a<BuffersForA, gemm_kernel<BuffersForC>>>>>>>;
