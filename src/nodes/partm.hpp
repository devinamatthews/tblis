...
template <int Dim, blocksize config::* BS, typename Child> struct partition
{
    Child child;
                    ...
                    abstract_matrix& C)
    {
        ...
        const len_type M_max =
            M.max(A.type()); // Equal to M_def for register block sizes
        const len_type M_iota =
            M.iota(A.type()); // Equal to the corresponding register block size
...
using partition_gemm_mc =
    partition<matrix_constants::DIM_M, &config::gemm_mc, Child>;

template <typename Child>
using partition_gemm_mr =
    partition<matrix_constants::DIM_M, &config::gemm_mr, Child>;

template <typename Child>
using partition_gemm_nc =
    partition<matrix_constants::DIM_N, &config::gemm_nc, Child>;

template <typename Child>
using partition_gemm_nr =
    partition<matrix_constants::DIM_N, &config::gemm_nr, Child>;

template <typename Child>
using partition_gemm_kc =
    partition<matrix_constants::DIM_K, &config::gemm_kc, Child>;
