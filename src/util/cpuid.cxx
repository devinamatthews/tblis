...
int get_cpu_type(int& family, int& model, int& features)
{
    ...
    if (cpuid_max >= 7)
    {
        ...
        if (check_features(ebx, FEATURE_MASK_AVX2))
            features |= FEATURE_AVX2;
        if (check_features(ebx, FEATURE_MASK_AVX512F))
            features |= FEATURE_AVX512F;
        if (check_features(ebx, FEATURE_MASK_AVX512PF))
            features |= FEATURE_AVX512PF;
        if (check_features(ebx, FEATURE_MASK_AVX512DQ))
            features |= FEATURE_AVX512DQ;
        if (check_features(ebx, FEATURE_MASK_AVX512BW))
            features |= FEATURE_AVX512BW;
        if (check_features(ebx, FEATURE_MASK_AVX512VL))
    ...
    if (cpuid_max_ext >= 0x80000001u)
    {
        ...
        if (check_features(ecx, FEATURE_MASK_FMA4))
    ...
    if (check_features(ecx, FEATURE_MASK_SSE3))
        features |= FEATURE_SSE3;
    if (check_features(ecx, FEATURE_MASK_SSSE3))
        features |= FEATURE_SSSE3;
    if (check_features(ecx, FEATURE_MASK_SSE41))
        features |= FEATURE_SSE41;
    if (check_features(ecx, FEATURE_MASK_SSE42))
        features |= FEATURE_SSE42;
    if (check_features(ecx, FEATURE_MASK_AVX))
        features |= FEATURE_AVX;
    if (check_features(ecx, FEATURE_MASK_FMA3))
    ...
    if (check_features(ecx, FEATURE_MASK_XGETBV))
    {
        ...
        // XGETBV_MASK_XMM)); fprintf(stderr, "xgetbv: ymm: %d\n",
        ...
        if (!check_features(
                eax,
                XGETBV_MASK_XMM | XGETBV_MASK_YMM | XGETBV_MASK_ZMM))
        {
            features &= ~(FEATURE_AVX512F
                          | FEATURE_AVX512PF
                          | FEATURE_AVX512DQ
        ...
        if (!check_features(eax, XGETBV_MASK_XMM | XGETBV_MASK_YMM))
        {
            features &=
    ...
    // FEATURE_AVX512F)); fprintf(stderr, "avx512pf: %d\n",
    // check_features(features, FEATURE_AVX512PF)); fprintf(stderr, "avx512dq:
    // %d\n", check_features(features, FEATURE_AVX512DQ));

    if (strcmp(reinterpret_cast<char*>(&vendor_string[0]), "AuthenticAMD") == 0)
        return VENDOR_AMD;
    else if (strcmp(reinterpret_cast<char*>(&vendor_string[0]), "GenuineIntel")
             == 0)
        return VENDOR_INTEL;
    else
        return VENDOR_UNKNOWN;
