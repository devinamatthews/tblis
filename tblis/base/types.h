...
typedef struct tblis_scalar
{
    ...
    {
        if (TBLIS_LIKELY(type == other.type)) memcpy(&data, &other.data, sizeof(data));
        else if (type & DOUBLE) data.z = other.data.c;
        else                    data.c = other.data.z;
    ...
    tblis_scalar& negate()
    {
        switch (type)
        {
            case FLOAT:    data.s = -data.s; break;
            case DOUBLE:   data.d = -data.d; break;
            case SCOMPLEX: data.c = -data.c; break;
            case DCOMPLEX: data.z = -data.z; break;
        }
...
#ifdef MARRAY_MARRAY_VIEW_HPP

template <typename T, int N>
struct tensor::convert<MArray::marray_view<T,N>>
{
    constexpr static bool is_mutable = !std::is_const<T>::value;

    void operator()(tensor& t, const MArray::marray_view<T,N>& other) const
    {
        t.scalar.reset(T(1));
...
template <typename T, int N>
struct tensor::convert<const MArray::marray_view<T,N>> : tensor::convert<MArray::marray_view<T,N>> {};

#endif //MARRAY_MARRAY_VIEW_HPP

#ifdef MARRAY_MARRAY_HPP

template <typename T, int N, typename A>
struct tensor::convert<MArray::marray<T,N,A>>
{
    constexpr static bool is_mutable = true;

    void operator()(tensor& t, MArray::marray<T,N,A>& other) const
    {
        t.scalar.reset(T(1));
        t.data = other.data();
        t.ndim = other.dimension();
...
template <typename T, int N, typename A>
struct tensor::convert<const MArray::marray<T,N,A>> : tensor::convert<MArray::marray<T,N,A>>
{
...
#ifdef MARRAY_VARRAY_VIEW_HPP

template <typename T>
struct tensor::convert<MArray::varray_view<T>>
{
    constexpr static bool is_mutable = !std::is_const<T>::value;

    void operator()(tensor& t, const MArray::varray_view<T>& other) const
    {
        t.scalar.reset(T(1));
        t.data = const_cast<void*>(static_cast<const void*>(other.data()));
        t.ndim = other.dimension();
...
template <typename T>
struct tensor::convert<const MArray::varray_view<T>> : tensor::convert<MArray::varray_view<T>> {};

#endif //MARRAY_VARRAY_VIEW_HPP

#ifdef MARRAY_VARRAY_HPP

template <typename T, typename A>
struct tensor::convert<MArray::varray<T,A>>
{
    constexpr static bool is_mutable = true;

    void operator()(tensor& t, MArray::varray<T,A>& other) const
    {
        t.scalar.reset(T(1));
        t.data = other.data();
        t.ndim = other.dimension();
...
template <typename T, typename A>
struct tensor::convert<const MArray::varray<T,A>> : tensor::convert<MArray::varray<T,A>>
{
    constexpr static bool is_mutable = false;
};

#endif //MARRAY_VARRAY_HPP
