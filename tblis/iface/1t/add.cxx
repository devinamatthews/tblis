...
void add(const communicator& comm,
         T alpha, dpd_varray_view<const T> A, const label_string& idx_A_,
         T  beta, dpd_varray_view<      T> B, const label_string& idx_B_)
{
    ...
    if (alpha == T(0) || (idx_A_only.empty() && idx_B_only.empty() && A.irrep() != B.irrep()))
    {
        ...
        if (beta == T(0))
        {
            internal::set(type_tag<T>::value, comm, get_default_config(),
                          beta, reinterpret_cast<dpd_varray_view<char>&>(B), idx_B_B);
        }
        else
        {
            internal::scale(type_tag<T>::value, comm, get_default_config(),
                            beta, false, reinterpret_cast<dpd_varray_view<char>&>(B), idx_B_B);
        }
    }
    else
    {
...
template void add(const communicator& comm, \
                   T alpha, dpd_varray_view<const T> A, const label_string& idx_A, \
                   T  beta, dpd_varray_view<      T> B, const label_string& idx_B);
#include <tblis/internal/foreach_type.h>

template <typename T>
void add(const communicator& comm,
         T alpha, indexed_varray_view<const T> A, const label_string& idx_A_,
         T  beta, indexed_varray_view<      T> B, const label_string& idx_B_)
{
    int ndim_A = A.dimension();
    int ndim_B = B.dimension();
    ...
    if (alpha == T(0))
    {
        ...
        {
            internal::set(type_tag<T>::value, comm, get_default_config(),
                          beta, reinterpret_cast<indexed_varray_view<char>&>(B), idx_B_B);
        }
        else
        {
            internal::scale(type_tag<T>::value, comm, get_default_config(),
                            beta, false, reinterpret_cast<indexed_varray_view<char>&>(B), idx_B_B);
        }
    }
    else
    {
...
#define FOREACH_TYPE(T) \
template void add(const communicator& comm, \
                   T alpha, indexed_varray_view<const T> A, const label_string& idx_A, \
                   T  beta, indexed_varray_view<      T> B, const label_string& idx_B);
#include <tblis/internal/foreach_type.h>

template <typename T>
void add(const communicator& comm,
         T alpha, indexed_dpd_varray_view<const T> A, const label_string& idx_A_,
         T  beta, indexed_dpd_varray_view<      T> B, const label_string& idx_B_)
{
    auto nirrep = A.num_irreps();
    TBLIS_ASSERT(B.num_irreps() == nirrep);
    ...
    if (alpha == T(0) || (idx_A_only.empty() && idx_B_only.empty() && A.irrep() != B.irrep()))
    {
        ...
        {
            internal::set(type_tag<T>::value, comm, get_default_config(),
                          beta, reinterpret_cast<indexed_dpd_varray_view<char>&>(B), idx_B_B);
        }
        else
        {
            internal::scale(type_tag<T>::value, comm, get_default_config(),
                            beta, false, reinterpret_cast<indexed_dpd_varray_view<char>&>(B), idx_B_B);
        }
    }
    else
    {
...
#define FOREACH_TYPE(T) \
template void add(const communicator& comm, \
                   T alpha, indexed_dpd_varray_view<const T> A, const label_string& idx_A, \
                   T  beta, indexed_dpd_varray_view<      T> B, const label_string& idx_B);
#include <tblis/internal/foreach_type.h>
