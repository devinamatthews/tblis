...
template <typename T>
void reduce(const communicator& comm, reduce_t op, MArray::dpd_varray_view<const T> A,
            const label_string& idx_A, T& result, len_type& idx);

template <typename T>
void reduce(reduce_t op, MArray::dpd_varray_view<const T> A, const label_string& idx_A,
...
template <typename T>
reduce_result<T> reduce(reduce_t op, MArray::dpd_varray_view<const T> A, const label_string& idx_A)
{
...
reduce_result<T> reduce(const communicator& comm, reduce_t op,
                        MArray::dpd_varray_view<const T> A, const label_string& idx_A)
{
...
template <typename T>
void reduce(const communicator& comm, reduce_t op, MArray::indexed_varray_view<const T> A,
            const label_string& idx_A, T& result, len_type& idx);

template <typename T>
void reduce(reduce_t op, MArray::indexed_varray_view<const T> A, const label_string& idx_A,
            T& result, len_type& idx)
{
    parallelize
    (
...
template <typename T>
reduce_result<T> reduce(reduce_t op, MArray::indexed_varray_view<const T> A, const label_string& idx_A)
{
    reduce_result<T> result(type_tag<T>::value);
    reduce(op, A, idx_A, result.value, result.idx);
    return result;
...
template <typename T>
reduce_result<T> reduce(const communicator& comm, reduce_t op,
                        MArray::indexed_varray_view<const T> A, const label_string& idx_A)
{
    reduce_result<T> result;
    reduce(comm, op, A, idx_A, result.value, result.idx);
    return result;
...
template <typename T>
void reduce(const communicator& comm, reduce_t op, MArray::indexed_dpd_varray_view<const T> A,
            const label_string& idx_A, T& result, len_type& idx);

template <typename T>
void reduce(reduce_t op, MArray::indexed_dpd_varray_view<const T> A, const label_string& idx_A,
            T& result, len_type& idx)
{
    parallelize
    (
...
template <typename T>
reduce_result<T> reduce(reduce_t op, MArray::indexed_dpd_varray_view<const T> A, const label_string& idx_A)
{
    reduce_result<T> result(type_tag<T>::value);
    reduce(op, A, idx_A, result.value, result.idx);
    return result;
...
template <typename T>
reduce_result<T> reduce(const communicator& comm, reduce_t op,
                        MArray::indexed_dpd_varray_view<const T> A, const label_string& idx_A)
{
    reduce_result<T> result;
    reduce(comm, op, A, idx_A, result.value, result.idx);
    return result;
...
struct data_type_helper
{
    ...
    #ifdef MARRAY_DPD_VARRAY_BASE_HPP

    template <typename T, typename D, bool O>
    static std::decay_t<T> check(MArray::dpd_varray_base<T,D,O>&);

    #endif //MARRAY_DPD_VARRAY_BASE_HPP

    #ifdef MARRAY_INDEXED_VARRAY_BASE_HPP

    template <typename T, typename D, bool O>
    static std::decay_t<T> check(MArray::indexed_varray_base<T,D,O>&);

    #endif //MARRAY_INDEXED_VARRAY_BASE_HPP

    #ifdef MARRAY_INDEXED_DPD_VARRAY_BASE_HPP

    template <typename T, typename D, bool O>
    static std::decay_t<T> check(MArray::indexed_dpd_varray_base<T,D,O>&);

    #endif //MARRAY_INDEXED_DPD_VARRAY_BASE_HPP
