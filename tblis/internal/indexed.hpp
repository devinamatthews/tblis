#ifndef TBLIS_INTERNAL_INDEXED_HPP
#define TBLIS_INTERNAL_INDEXED_HPP
...
struct group_indices : std::vector<index_set<N>>
{
    ...
    group_indices(type_t type, const Array& A, const Args&... args)
    {
        ...
        for (len_type i = 0;i < A.num_indices();i++)
        {
            ...
            switch (type)
            {
                case FLOAT:    idx.factor.reset(reinterpret_cast<const    float*>(&A.factor(0))[i]); break;
                case DOUBLE:   idx.factor.reset(reinterpret_cast<const   double*>(&A.factor(0))[i]); break;
                case SCOMPLEX: idx.factor.reset(reinterpret_cast<const scomplex*>(&A.factor(0))[i]); break;
                case DCOMPLEX: idx.factor.reset(reinterpret_cast<const dcomplex*>(&A.factor(0))[i]); break;
            }
...
#endif //TBLIS_INTERNAL_INDEXED_HPP
