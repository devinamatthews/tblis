...
{
    const MArray::dpd_varray_view<char>& tensor_;
    ...
    dpd_tensor_matrix_impl(const MArray::dpd_varray_view<char>& other,
...
struct is_dpd_tensor_helper<MArray::dpd_varray<T,Allocator>> : std::true_type {};

template <typename T>
struct is_dpd_tensor_helper<MArray::dpd_varray_view<T>> : std::true_type {};
...
class dpd_tensor_matrix : public abstract_matrix_adapter<dpd_tensor_matrix,dpd_tensor_matrix_impl>
{
    ...
    public:
        dpd_tensor_matrix(const tblis_scalar& alpha, bool conj,
                          const MArray::dpd_varray_view<char>& other,
                          const dim_vector& row_inds,
                          const dim_vector& col_inds,
                          int col_irrep,
                          const dim_vector& extra_inds,
        ...
        const MArray::dpd_varray_view<char>& tensor() const
        {
            return impl().tensor_;
        }
