...
TEMPLATED_TEST_CASE(gemm_ukr, T, all_types)
{
    for (auto i : range(num_configs))
    {
        ...
        INFO_OR_PRINT("row major? "
                      << (cfg.gemm_row_major.value<T>() ? "yes" : "no"));
        INFO_OR_PRINT("flipped? "
                      << (cfg.gemm_flip_ukr.value<T>() ? "yes" : "no"));
        ...
        std::array<len_type, 8> ks =
            {0, 1, KC - 1, KC, KC + 1, KX - 1, KX, KX + 1};
            ...
            for (auto k : ks)
            {
                ...
                for (auto stride : strides)
                {
                    ...
                    for (T beta : {0, 1, -1})
                    {
                        ...
                        matrix_view<T> A({m, k},
                                         reinterpret_cast<T*>(A_buf),
                                         {1, ME});
                        matrix_view<T> B({k, n},
                                         reinterpret_cast<T*>(B_buf),
                                         {NE, 1});
                        matrix_view<T> C({m, n},
                                         reinterpret_cast<T*>(C_buf),
                                         {rs_c, cs_c});
                        matrix_view<T> D({m, n},
                                         reinterpret_cast<T*>(D_buf),
                                         {rs_c, cs_c});

                        packed_matrix Ap(type_tag<T>::value,
                                         MR,
                                         k,
                                         A_buf,
                                         ME * k);
                        packed_matrix Bp(type_tag<T>::value,
                                         k,
                                         NR,
                                         B_buf,
                                         NE * k);
