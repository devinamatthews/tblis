...
template <typename T>
using copy_p =
    void (*)(const int* n, const T* A, const int* inca, T* B, const int* incb);

template <typename T> using gemm_f = typename remove_pointer<gemm_p<T>>::type;

template <typename T> using copy_f = typename remove_pointer<copy_p<T>>::type;
...
template <> struct copy_ptr<float>
{
    constexpr static copy_p<float> value = &scopy_;
};

template <> struct copy_ptr<double>
{
    constexpr static copy_p<double> value = &dcopy_;
};

template <> struct copy_ptr<scomplex>
{
    constexpr static copy_p<scomplex> value = &ccopy_;
};

template <> struct copy_ptr<dcomplex>
{
    constexpr static copy_p<dcomplex> value = &zcopy_;
};
...
template <typename T> void copy(int n, const T* A, int lda, T* B, int ldb)
{
...
template <> struct type_char<float>
{
    static constexpr char value = 's';
};

template <> struct type_char<double>
{
    static constexpr char value = 'd';
};

template <> struct type_char<scomplex>
{
    static constexpr char value = 'c';
};

template <> struct type_char<dcomplex>
{
    static constexpr char value = 'z';
};

template <typename T, algo_t Algorithm> struct gemm_experiment
{
    len_type R;
...
struct random_contraction
{
    ...
    void operator()(const map<char, len_type>& values) const
    {
        ...
        {
            auto len_m =
                random_product_constrained_sequence<len_type, ROUND_NEAREST>(
                    random_number(1, 3),
                    m);
            auto len_n =
                random_product_constrained_sequence<len_type, ROUND_NEAREST>(
            ...
            tblis::internal::impl =
                (Implementation == BLAS ? tblis::internal::BLAS_BASED
            ...
            printf("%e %e -- %s %c %*s %*s %*s",
                   gflops,
                   gflops / dt,
                   Implementation == BLIS ? "rand_blis" : "rand_blas",
                   type_char<T>::value,
                   static_cast<int>(idx_A.size()),
                   idx_A.data(),
                   static_cast<int>(idx_B.size()),
                   idx_B.data(),
                   static_cast<int>(idx_C.size()),
                   idx_C.data());
...
template <typename T, algo_t Implementation> struct regular_contraction
{
    len_type R;
    label_vector idx_A, idx_B, idx_C;
    ...
    void operator()(const map<char, len_type>& lengths) const
    {
        ...
        for (char c : idx_A) { len_A.push_back(lengths.at(c)); }

        for (char c : idx_B) { len_B.push_back(lengths.at(c)); }

        for (char c : idx_C) { len_C.push_back(lengths.at(c)); }
    ...
    while (getline(cin, line) && !line.empty())
    {
            ...
            == "rand_blis")
        {
            ...
            switch (dt)
            {
                case 's':
                    if (algo == "blis")
                        gemm_experiment<float, BLIS>(R, m, n, k);
                    else if (algo == "blis+copy")
                        gemm_experiment<float, BLIS_COPY>(R, m, n, k);
                    else if (algo == "blas")
                        gemm_experiment<float, BLAS>(R, m, n, k);
                    else if (algo == "blas+copy")
                        gemm_experiment<float, BLAS_COPY>(R, m, n, k);
                    else if (algo == "rand_blis")
                        random_contraction<float, BLIS>(R, m, n, k);
                    else if (algo == "rand_blas")
                        random_contraction<float, BLAS>(R, m, n, k);
                    break;
                case 'd':
                    if (algo == "blis")
                        gemm_experiment<double, BLIS>(R, m, n, k);
                    else if (algo == "blis+copy")
                        gemm_experiment<double, BLIS_COPY>(R, m, n, k);
                    else if (algo == "blas")
                        gemm_experiment<double, BLAS>(R, m, n, k);
                    else if (algo == "blas+copy")
                        gemm_experiment<double, BLAS_COPY>(R, m, n, k);
                    else if (algo == "rand_blis")
                        random_contraction<double, BLIS>(R, m, n, k);
                    else if (algo == "rand_blas")
                        random_contraction<double, BLAS>(R, m, n, k);
                    break;
                case 'c':
                    if (algo == "blis")
                        gemm_experiment<scomplex, BLIS>(R, m, n, k);
                    else if (algo == "blis+copy")
                        gemm_experiment<scomplex, BLIS_COPY>(R, m, n, k);
                    else if (algo == "blas")
                        gemm_experiment<scomplex, BLAS>(R, m, n, k);
                    else if (algo == "blas+copy")
                        gemm_experiment<scomplex, BLAS_COPY>(R, m, n, k);
                    else if (algo == "rand_blis")
                        random_contraction<scomplex, BLIS>(R, m, n, k);
                    else if (algo == "rand_blas")
                        random_contraction<scomplex, BLAS>(R, m, n, k);
                    break;
                case 'z':
                    if (algo == "blis")
                        gemm_experiment<dcomplex, BLIS>(R, m, n, k);
                    else if (algo == "blis+copy")
                        gemm_experiment<dcomplex, BLIS_COPY>(R, m, n, k);
                    else if (algo == "blas")
                        gemm_experiment<dcomplex, BLAS>(R, m, n, k);
                    else if (algo == "blas+copy")
                        gemm_experiment<dcomplex, BLAS_COPY>(R, m, n, k);
                    else if (algo == "rand_blis")
                        random_contraction<dcomplex, BLIS>(R, m, n, k);
                    else if (algo == "rand_blas")
                        random_contraction<dcomplex, BLAS>(R, m, n, k);
                    break;
            }
        }
        else if (algo == "reg_blas" || algo == "reg_blis")
        {
            ...
            switch (dt)
            {
                case 's':
                    if (algo == "reg_blis")
                        regular_contraction<float, BLIS>(R,
                                                         idx_A,
                                                         idx_B,
                                                         idx_C,
                                                         ranges);
                    else if (algo == "reg_blas")
                        regular_contraction<float, BLAS>(R,
                                                         idx_A,
                                                         idx_B,
                                                         idx_C,
                                                         ranges);
                ...
                case 'd':
                    if (algo == "reg_blis")
                        regular_contraction<double, BLIS>(R,
                                                          idx_A,
                                                          idx_B,
                                                          idx_C,
                                                          ranges);
                    else if (algo == "reg_blas")
                        regular_contraction<double, BLAS>(R,
                                                          idx_A,
                                                          idx_B,
                                                          idx_C,
                                                          ranges);
                ...
                case 'c':
                    if (algo == "reg_blis")
                        regular_contraction<scomplex, BLIS>(R,
                                                            idx_A,
                                                            idx_B,
                                                            idx_C,
                                                            ranges);
                    else if (algo == "reg_blas")
                        regular_contraction<scomplex, BLAS>(R,
                                                            idx_A,
                                                            idx_B,
                                                            idx_C,
                                                            ranges);
                ...
                case 'z':
                    if (algo == "reg_blis")
                        regular_contraction<dcomplex, BLIS>(R,
                                                            idx_A,
                                                            idx_B,
                                                            idx_C,
                                                            ranges);
                    else if (algo == "reg_blas")
                        regular_contraction<dcomplex, BLAS>(R,
                                                            idx_A,
                                                            idx_B,
                                                            idx_C,
                                                            ranges);
