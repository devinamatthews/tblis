...
double diff(const dpd_marray_view<T>& A, const dpd_marray_view<T>& B)
{
    ...
    for (stride_type i = 0; i < size; i++) { d += norm2(a[i] - b[i]); }

    return sqrt(d);
}

template <typename T> void randomize(dpd_marray<T>& A)
{
    T* a = A.data();
    stride_type size = dpd_marray<T>::size(A.irrep(), A.lengths());

    for (stride_type i = 0; i < size; i++) { a[i] = random_number<double>(); }
}

template <typename T>
void bench(int R,
           T alpha,
           const dpd_marray<T>& A,
           const std::string& typea,
           const dpd_marray<T>& B,
...
int main(int argc, char** argv)
{
    ...
    if (test0)
    {
        ...
        bench(R, 1.0, A, "AE", B, "EB", 1.0, C, "AB");
    }
    ...
    if (test1)
    {
        ...
        bench(R, 1.0, A, "AEIM", B, "EBMJ", 1.0, C, "ABIJ");
    }
